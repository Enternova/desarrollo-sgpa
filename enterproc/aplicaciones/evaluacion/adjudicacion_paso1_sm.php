...
<table width="99%" border="0" cellpadding="1" cellspacing="1" class="tabla_lista_resultados">
              ...
              <tr>
                ...
                <td width="13%" class="columna_titulo_resultados"><div align="center"><strong>Numero de OS</strong></div></td>
                <td width="14%" class="columna_titulo_resultados"><div align="center"><strong>Gerente  OS</strong></div></td>
                     <? if ($estado_proceso==5){ ?>
  ...
  <tr class="<?=$class;?>">
    <td><?=$lp[2];?><input type="hidden" name="nombre_provee_<?=$lp[0];?>" id="nombre_provee_<?=$lp[0];?>" value="<?=$lp[2];?>">
      <input type="hidden" name="cargo_contable_<?=$lp[0];?>" id="cargo_contable_<?=$lp[0];?>">
    <input type="hidden" name="fecha_entrega_<?=$lp[0];?>" id="fecha_entrega_<?=$lp[0];?>" value="n/a" ></td>
    <td><input type="text" name="documento_<?=$lp[0];?>" id="documento_<?=$lp[0];?>"></td>
                <td><select name="contacto_<?=$lp[0];?>" id="contacto_<?=$lp[0];?>">
                  <? 
    ...
    <td><input type="text" name="numeroaprob_<?=$lp[0];?>" id="numeroaprob_<?=$lp[0];?>"></td>
                ...
                <td align="center"><input type="button" name="button" class="guardar" id="button" value="Adjudicar" onClick="crea_adju_provee_servico_menor(<?=$lp[0];?>)"></td>
...
<table width="99%" border="0" cellpadding="1" cellspacing="1" class="tabla_lista_resultados">
  <tr>
    <td colspan="4" class="columna_subtitulo_resultados"><strong>Proveedores seleccionados para adjudicaci&oacute;n</strong></td>
  </tr>
  <tr>
    <td width="34%" class="columna_titulo_resultados"><div align="center"><strong>Nombre proveedor</strong></div></td>
                <? if ($estado_proceso==5){ ?>
    ...
    <td width="14%" class="columna_titulo_resultados"><strong>Numero de OS</strong></td>
    <td width="18%" class="columna_titulo_resultados"><div align="center"><strong>Gerente OS</strong></div></td>
  ...
  ?>
  <tr class="<?=$class;?>">
    <td><?=$lp[3];?><input type="hidden" name="nombre_provee_<?=$lp[2];?>" id="nombre_provee_<?=$lp[2];?>" value="<?=$lp[3];?>">
    <input type="hidden" name="cargo_contable_<?=$lp[2];?>" id="cargo_contable_<?=$lp[2];?>" value="<?=$lp[9];?>">
    <input type="hidden" name="fecha_entrega_<?=$lp[2];?>" id="fecha_entrega_<?=$lp[2];?>" value="<?=$lp[5];?>" onMouseDown="calendario_sin_hora('fecha_entrega_<?=$lp[2];?>')"></td>
  <? if ($estado_proceso==5){ ?>
    <td><?=$lp[9];?></td>
    <? } ?>    
    			<td><input type="text" name="documento_<?=$lp[2];?>" id="documento_<?=$lp[2];?>" value="<?=$lp[4];?>"></td>
                <td><select name="contacto_<?=$lp[2];?>" id="contacto_<?=$lp[2];?>">
                  <? 
					if($_SESSION["pv_principal"] != 150)
						$traer_tecnicos_compras = " tipo_usuario <> 2 and tipo_usuario <> 10 and estado = 1 and us_id <> ".$_SESSION["id_us_session"]." ";
					elseif($_SESSION["pv_principal"] == 150)
					...
					?>
                  <?=listas_mayus($t1, $traer_tecnicos_compras,$lp[6],'nombre_administrador', 1);?>
                </select>
                
                </td>
                <td align="center"><img src="../imagenes/botones/editar_c.png"  title="Editar datos de adjudicación" alt="Editar datos de adjudicaci&oacute;n" width="16" height="16" longdesc="Editar datos de adjudicación" onClick="edita_adju_provee_sm(<?=$lp[2];?>)"> 
                 <img src="../imagenes/botones/b_cancelar.gif" alt="Descincular proveedor adjudicado" title="Descincular proveedor adjudicado" width="16" height="16" longdesc="Descincular proveedor adjudicado" onClick="elimina_adju_provee_sm(<?=$lp[2];?>)"></td>
