...
$carpeta_q=elimina_comillas_2_archivos($sql_proceso[0]."_comerciales");

$carpeta = substr($carpeta_q, 0, 20);
...
$busca_provee = query_db("select $t8.pv_id, $t8.nit, $t8.razon_social, $t8.telefono, $t8.email, $t8.estado_rup from $t8, $t7 where
				$t7.pro1_id =  $evaluador1_id and $t8.pv_id = $t7.pv_id  and $t7.pv_id = $pv_id ");
				while($lp = traer_fila_row($busca_provee)){
				$cuenta_archivos = traer_fila_row(query_db("select count(*) from ".$v10." where pro1_id = $evaluador1_id and pv_id = $lp[0] and  termino = 1"));
...
$busca_respo = query_db("select evaluador6_id, evaluador6_nombre,razon_social from ".$v10." where pro1_id = $evaluador1_id and termino = 1 and  pv_id = $pv_id  ");
