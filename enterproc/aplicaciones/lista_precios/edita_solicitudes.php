...
<table width="98%" border="0" cellspacing="2" cellpadding="2">
  <tr>
    <td width="62%"><fieldset style="width:98%">
      ...
      <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tabla_paginador">
        <tr>
          ...
          <td width="7%" class="columna_subtitulo_resultados_economico"><div align="center"><a href="javascript:busqueda_paginador_nuevo(<?=$anterior;?>,'../aplicaciones/lista_precios/edita_solicitudes.php','contenidos', '<?=$tipo_ingreso_alerta;?>')">Anterior</a></div></td>
          <td width="1%" class="columna_subtitulo_resultados_economico"><label>
            <select name="pagij" onchange="javascript:busqueda_paginador_nuevo(this.value,'../aplicaciones/lista_precios/edita_solicitudes.php','contenidos', '<?=$tipo_ingreso_alerta;?>')">
              <? 
		  for($i=1;$i<=$pagina;$i++){
		   ?>
          ...
          <td width="6%" class="columna_subtitulo_resultados_economico"><a href="javascript:busqueda_paginador_nuevo(<?=$proxima;?>,'../aplicaciones/lista_precios/edita_solicitudes.php','contenidos', '<?=$tipo_ingreso_alerta;?>')">Siguiente</a></td>
        </tr>
      </table>
...
?>
    ...
    <td width="38%" valign="top"><fieldset style="width:98%">
...
<br />
  ...
  <tr class="<?=$class;?>">
    ...
    <td><div align="left"><?=$lp[2];?>
                ...
                <td><div align="left"><a href="javascript:void(0)" onclick="abre_proveedores('carga_otros_<?=$lp[0];?>', <?=$lp[9];?>, '<?=$lp[0];?>' )"><?=$lp[5];?></a></div></td>
                <td><div align="center">
                  <?=$lp[4];?>
                </div></td>
                <td>
  ...
  <tr class="<?=$class;?>"><td colspan="9" id="carga_otros_<?=$lp[0];?>"></td></tr>
...
<fieldset style="width:98%">
		   ...
		   $busca_solicitud = traer_fila_row(query_db("select * from $l4 where lista4_id = $id_compra"));
		...
		$l5.lista4_id  = $id_compra and $l6.lista6_id = $l5.lista6_id  and
		...
		$fecha_mes_hoy = date("m", strtotime($busca_solicitud[2]));
		$fecha_ano_hoy = date("Y", strtotime($busca_solicitud[2]));		

		if($bb[2]=="Diaria")
			$comple = " and DATE($l4.fecha_creacion) = '$fecha' ";
		elseif($bb[2]=="Mensual")
		...
		if($resta_menos_esta_compra<0)
			{
				...
				if($accion=="notifica")
				 $nota.="La notificación NO se envio <br>";
...
<table width="95%" border="0" cellspacing="2" cellpadding="2">
              <tr>
                ...
                <td>
                  <div align="center">
                    <input name="button4" type="button" class="guardar" id="button4" value="Grabar y notificar  a los proveedores" onclick="notifica_prove_lista(<?=$id_compra;?>)" />
                  </div></td>
                <td><div align="center">
					...
					if($accion=="notifica")
						{
							...
							$insert = query_db("update $l4 set estado = 2 where lista4_id = $id_compra");							
