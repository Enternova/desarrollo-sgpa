...
<table width="95%" border="0" cellspacing="3" cellpadding="3">
  ...
  <tr>
	...
	else echo  campo_consecutivo($sql_e[22]);
			?>
...
<? if($sql_e[7]>=1) { echo listas_sin_select($tp3,$sql_e[3],1);
  ...
  <tr>
    ...
    <td width="29%"><div align="left">
 	...
 	elseif($sql_e[2]==200) { echo listas_sin_select($tp2,$sql_e[2],1);
      ...
      <select name="a" id="a" >
      <? if($id_proceso==""){echo "<option value='30'>Sondeo de Mercado</option>";}else{?>
  ...
  <tr>
    ...
    <td colspan="3">
      <div align="left">
        <textarea name="d" cols="1" rows="2" id="d"><?=$sql_e[12];?></textarea>
      </div>    </td>
  </tr>
</table>
<br />
...
<fieldset style="width:98%">
            ...
            <table width="98%" border="0" cellpadding="3" cellspacing="3" class="tabla_cronograma" >
              <tr>
                ...
                <td width="11%" class="titulo_tabla_azul_sin_bordes">% Minimo aceptado</td>
              ...
              <tr class="campos_gris_cronograma">
                ...
                <td ><select name="responsable_tec" id="responsable_tec">
                  <?=listas_mayus($t1, " tipo_usuario <> 2 and tipo_usuario <> 10 and estado = 1 and us_id <> ".$_SESSION["id_us_session"]." ",$busca_responsable_t[1],'nombre_administrador', 1);?>
                </select></td>
              </tr>
              ...
              <tr class="campos_blancos_cronograma">
                ...
                <td><select name="respo_juridico" id="respo_juridico">
                    <?=listas_mayus($t1, " tipo_usuario <> 2 and tipo_usuario <> 10 and  estado = 1",$busca_responsable_j[1],'nombre_administrador', 1);?>
                </select></td>
              </tr>
              <tr class="campos_blancos_cronograma">
                <td>&nbsp;</td>
              ...
              <tr class="campos_gris_cronograma">
                ...
                <td><select name="responsable_ec" id="responsable_ec">
                  <?=listas_mayus($t1, " tipo_usuario <> 2 and tipo_usuario <> 10 and estado = 1",$busca_responsable_e[1],'nombre_administrador', 1);?>
                                </select></td>
              </tr>
            </table>
<br />
...
<table width="95%" border="0" cellspacing="2" cellpadding="2">
 ...
 <?=crear_lugar_ejecucion($id_proceso);?>
  <tr>
    ...
    <td><div align="left">
      <select name="k" id="k">
        <?=listas_mayus($t1, " tipo_usuario <> 2 and tipo_usuario <> 10 and us_id <> 597 and estado =1",$busca_contacto,'nombre_administrador', 1);?>
      </select>
    </div></td>
    <td width="269" colspan="-1"><div align="right"><strong>Entrega de documentos fisicos:</strong></div></td>
    ...
    <td>
      <div align="center">
        <? if($sql_e[31]==0){ //verifica si no fue invitada
         	...
         	if($id_proceso==""){ ?>
		        <input name="button6" type="button" class="guardar" id="button6" value="Grabar Proceso" onclick="crea_proceso('<?=date('Y')?>','<?=date('n')?>','<?=date('j')?>','<?=date('h')?>','<?=date('i')?>')" />
			<? } else { ?>
            ...
            <table width="99%" border="0" cellspacing="2" cellpadding="2">
              ...
              <tr>
                ...
                <td><div align="left">
                    ...
                    <input name="proveedor" type="text" id="proveedor" size="50" onkeypress="selecciona_lista()" />                    </label>
                </div></td>
                <td width="26%">
                    <div align="left">
             ...
             <? if ( ($sql_e[31]==1) && ($sql_e[2]==1) ){ ?>
              ...
              <tr>
                <td><input name="button9" type="button" class="buscar" id="button9" value="Buscar proveedores en RUP PAR SERVICIOS" onclick="ajax_carga('../aplicaciones/crea_contactos_proveedor.php?id_invitacion_pasa=<?=$id_proceso;?>','muestra_cootactos');ingresar_listado('oculta_todo_proveedores')" /></td>
                <td><input name="button10" type="text" class="buscar" id="button10" value="Crear proveedor" onclick="ajax_carga('../aplicaciones/crea_contactos_proveedor_emergencia.php?id_invitacion_pasa=<?=$id_proceso;?>','muestra_cootactos');ingresar_listado('oculta_todo_proveedores')" /></td>
                <td><div align="right">
...
<tr>
  ...
  ?>
    ...
    <td><input name="button5" type="button" class="buscar" id="button5" value="Ver" onclick="ajax_carga('../aplicaciones/carga_contactos_proveedor.php?pv_nit=<?=$lp[1];?>','muestra_cootactos');ingresar_listado('oculta_todo_proveedores')" /></td>
                ...
                <td><div align="center"><a href="javascript:void(0)" onclick="elimina_proveedor(<?=$lp[0];?>)"><img src="../imagenes/botones/b_cancelar.gif" title="Eliminar Proveedor de la invitaci&oacute;n" /></a></div></td>
                <? 
