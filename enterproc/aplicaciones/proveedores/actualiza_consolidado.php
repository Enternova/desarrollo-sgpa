...
if($campo_mejor_oferta_semaforo!=""){//1
  	$busca_campos = query_db("select * from $t95 where in_id = $id_invitacion and pro11_id = $id_lista  limit $paginador,$numero_pagi ");
	while($l_campo = traer_fila_row($busca_campos)){ //while
		$busca_valores_ing_mejor_proveedor=traer_fila_row(query_db("select id_campo_seleccionado from reporte_temp1_$us_cliente  where  evaluador5_id  = $l_campo[0] and id_item_lista = $campo_mejor_oferta_semaforo and id_campo_seleccionado >=1 and pv_id = $us_cliente"));
		$valor_proveedor_buscado = ($busca_valores_ing_mejor_proveedor[0]*1);

		$sql = "select id_campo_seleccionado from reporte_temp1_$us_cliente  where  evaluador5_id  = $l_campo[0] and id_item_lista = $campo_mejor_oferta_semaforo and id_campo_seleccionado >=1 and pv_id = $us_cliente";
			
			$busca_valores_ing_mejor=traer_fila_row(query_db("select min(id_campo_seleccionado *1) from reporte_temp1_$us_cliente  where  evaluador5_id  = $l_campo[0] and id_item_lista = $campo_mejor_oferta_semaforo and id_campo_seleccionado >=1 "));
			$cuenta_valores_ing_mejor=traer_fila_row(query_db("select count(*) from reporte_temp1_$us_cliente  where  evaluador5_id  = $l_campo[0] and id_item_lista = $campo_mejor_oferta_semaforo and id_campo_seleccionado>=1 and id_campo_seleccionado=$busca_valores_ing_mejor[0] "));
			
			if(($valor_proveedor_buscado>=1) && ($valor_proveedor_buscado<=$busca_valores_ing_mejor[0])){//2
				if($cuenta_valores_ing_mejor[0]*1==1)
					$semaforo.="sema_".$l_campo[0]."--<img src='../imagenes/botones/SemaforoVerde.gif' alt='Usted tiene la mejor oferta en este articulo hasta el momento'>|";
				 else 
				 	$semaforo.="sema_".$l_campo[0]."--<img src='../imagenes/botones/SemaforoAmarilloAnimado.gif' alt='Usted tiene la oferta uno o varios oferentes hasta el momento'>|";
			}//2
			else 
				 	$semaforo.="sema_".$l_campo[0]."--<img src='../imagenes/botones/SemaforoRojo.gif' alt='Usted NO tiene la mejor oferta en este articulo hasta el momento'>|";
...
?>
<?=$tabla_semaforo_consolidado."$$".$semaforo;?>
