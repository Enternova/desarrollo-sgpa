...
if($_POST["accion"]=="crea_proceso")
	...
	elseif($a==31)
		...
		if( $ejucuta_b_espac[0]>=1)
            ...
            <script>
            	window.parent.muestra_alerta_error_solo_texto('', 'Error', '¡El consecutivo que intenta crear ya existe! <?=$conseutivo_arr;?>', 20, 10, 18);
...
/******************************************************************************************************/	
	...
	echo	$inserta_procesos="insert into $t5 (tp1_id,tp2_id,tp3_id,tp4_id,tp5_id,cd_id_ejecucion,cd_id_entrega_documentos,
		...
		minimo_tecnico_solicitado, peso_economico,consecutivo,apertura_juridica,cierre_juridica, apertura_tecnica, cierre_tecnica,apertura_economica, cierre_economica, fecha_informativa, lugar, fecha_creacion, us_id, fecha_informativa_final, fecha_aclaraciones_2_inicial, fecha_aclaraciones_2_final, fecha_aclaraciones_3_inicial, fecha_aclaraciones_3_final, fecha_preconomica_inicial, fecha_preconomica_final, nueva_fecha_informativa, trm_actual,origen_duplicidad,us_id_otro_contacto )
		 values (9, $a,$tipo_solicitud, $b, $g, 0,0,'$l',0,'$docu_fisi',$c, '$d', $f,  $e, $k,'$h','$i','$j',$p_t,$m_t,0,'".$conseutivo_arr."' ,'$a_j','$c_j','$a_t','$c_t','$a_e','$c_e', '$fecha_informativa','$direccion_info' , '$fecha $hora', ".$_SESSION["id_us_session"].", '$fecha_informativa_f','$a_j5','$a_j6','$a_j7','$a_j8','$a_e_p', '$c_e_p', '$fecha_reu_info', '$trm_actual',0,'$us_id_otro_contacto')";
...
echo $cambia_observacion;
		...
		if($tipo_aclaracion==0){
			$arr=$_POST["evaluacion_juridica"];
		}else{
			$arr=$_POST["evaluacion_juridica1"];
		}

		foreach($arr as $id_evaluacion_enviada => $valor_obtenido)
		{//recorre la evaluacion obtenida


			
			if($tipo_configuracion_tecnica==25){
				if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$valor_obtenido = $evaluacion_juridicacompleto;
				}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
					$valor_obtenido = $evaluacion_juridicacompleto1;
				}
			}
			...
			if($valor_obtenido!="0"){//si fue calificado
				if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$busca_exite = mysql_fetch_row(mysql_query("select * from $t96  where evaluador1_id = $id_evaluacion_enviada and pv_id = $pv_id and (tipo_aclaracion not in(2,3) or tipo_aclaracion is null)"));
				}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
					$busca_exite = mysql_fetch_row(mysql_query("select * from $t96  where evaluador1_id = $id_evaluacion_enviada and pv_id = $pv_id and tipo_aclaracion in(2,3)"));
				}
				if($busca_exite[0]>=1){
					if($tipo_aclaracion==0){//si se cargo por ofertas normales
						$cambia_resultado = "update $t96 set resultado_evaluacion = '$valor_obtenido' where evaluador1_id = $id_evaluacion_enviada and pv_id = $pv_id and tipo_aclaracion not in(2,3)";
					}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
						$cambia_resultado = "update $t96 set resultado_evaluacion = '$valor_obtenido' where evaluador1_id = $id_evaluacion_enviada and pv_id = $pv_id and tipo_aclaracion in(2,3)";
					}
				...
				$sql_ex = mysql_query($cambia_resultado);
					...
					if($tipo_aclaracion==0){//si se cargo por ofertas normales
						$insertar = mysql_query("insert into $t96 (pv_id, evaluador1_id, evaluador6_nombre, evaluador6_tamano, evaluador6_tipo, evaluador6_observaciones, evaluador6_fecha, resultado_evaluacion, resultado_evaluacion_afectado, tipo_aclaracion) values ($pv_id,$id_evaluacion_enviada, '',0,'','','','$valor_obtenido', '', 0)");
					}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
						$insertar = mysql_query("insert into $t96 (pv_id, evaluador1_id, evaluador6_nombre, evaluador6_tamano, evaluador6_tipo, evaluador6_observaciones, evaluador6_fecha, resultado_evaluacion, resultado_evaluacion_afectado, tipo_aclaracion) values ($pv_id,$id_evaluacion_enviada, '',0,'','','','$valor_obtenido', '', 3)");
					}
					
				}
				if($tipo_aclaracion_aprobacion!=0){
					if($tipo_evaluacion!=25){
						foreach($_POST["evaluacion_juridica1"] as $id_evaluacion_enviada2 => $valor_obtenido2){
							if($valor_obtenido2==2){
								$estado_n+=1;
							}
						}
					}else{
						if($evaluacion_juridicacompleto1==2){
								$estado_n+=1;
						}
					}
				}else{
					if($tipo_evaluacion!=25){
						...
						if($evaluacion_juridicacompleto==2){
								$estado_n+=1;
						}
					}
				}
				if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$busca_ob_evalu = mysql_fetch_row(mysql_query("select * from $t32 where evaluador1_id=$id_evaluacion_enviada and pv_id=$pv_id and tipo_aclaracion not in(2,3)"));
				}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
					$busca_ob_evalu = mysql_fetch_row(mysql_query("select * from $t32 where evaluador1_id=$id_evaluacion_enviada and pv_id=$pv_id and tipo_aclaracion in(2,3)"));
				}
					if($busca_ob_evalu[0]>=1)
						{
							$query3=$id_evaluacion_enviada;
							if($tipo_aclaracion_aprobacion==0){//si se cargo por ofertas normales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa_".$id_evaluacion_enviada]."' where evaluador1_id=$id_evaluacion_enviada and pv_id=$pv_id and tipo_aclaracion not in(2,3)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion==0){//si se cargo por aclaraciones finales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa_".$id_evaluacion_enviada]."' where evaluador1_id=$id_evaluacion_enviada and pv_id=$pv_id and tipo_aclaracion not in(2,3)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion!=0){//si se cargo por aclaraciones finales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa1_".$id_evaluacion_enviada]."' where evaluador1_id=$id_evaluacion_enviada and pv_id=$pv_id and tipo_aclaracion in(2,3)";
							}
					$sql_ob=mysql_query($cambia_observacion);
							...
							if($tipo_aclaracion_aprobacion==0){//si se cargo por ofertas normales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada,$pv_id,'".$_POST["observa_".$id_evaluacion_enviada]."', 0)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion==0){//si se cargo por aclaraciones finales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada,$pv_id,'".$_POST["observa_".$id_evaluacion_enviada]."', 0)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion!=0){//si se cargo por aclaraciones finales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada,$pv_id,'".$_POST["observa1_".$id_evaluacion_enviada]."', 3)";
							}
						$sql_ob=mysql_query($cambia_observacion);
			...
			if($tipo_aclaracion_aprobacion!=0){
				if($tipo_evaluacion!=25){
					foreach($_POST["evaluacion_juridica1"] as $id_evaluacion_enviada3 => $valor_obtenido2){
						if($valor_obtenido2=="Sin"){
							$estado_p+=1;
						}
					}
				}else{
					if($evaluacion_juridicacompleto1=="Sin"){
							$estado_p+=1;
					}
				}
			}else{
				if($tipo_evaluacion!=25){
					...
					if($evaluacion_juridicacompleto=="Sin"){
							$estado_p+=1;
					}
				}
			}
		...
		//echo $estado_n;
			...
			elseif($termino_eva==2){
				...
				if($_FILES["soporte_avluacion"]["name_tmp"]!="")
...
if($accion=="evaluacion_tecnica")
	...
	$estado_n="";
		...
		if($tipo_aclaracion==0){
			$arr1=$_POST["evaluacion_tecnica"];
		}else{
			$arr1=$_POST["evaluacion_tecnica1"];
		}
		foreach($arr1 as $id_evaluacion_enviada => $valor_obtenido)
		{//recorre la evaluacion obtenida
...
$id_proceso_unit_te = explode("_",$id_evaluacion_enviada);
			$id_evaluacion_enviada_1=$id_proceso_unit_te[0];
				if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$busca_ob_evalu = mysql_fetch_row(mysql_query("select * from $t32 where evaluador1_id=$id_evaluacion_enviada_1 and pv_id=$pv_id and (tipo_aclaracion not in(2,3) or tipo_aclaracion is null)"));
				}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
					$busca_ob_evalu = mysql_fetch_row(mysql_query("select * from $t32 where evaluador1_id=$id_evaluacion_enviada_1 and pv_id=$pv_id and tipo_aclaracion in(2,3)"));
				}
					if($busca_ob_evalu[0]>=1)
						{
							if($tipo_aclaracion_aprobacion==0){//si se cargo por ofertas normales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa_".$id_evaluacion_enviada_1]."' where evaluador1_id=$id_evaluacion_enviada_1 and pv_id=$pv_id and tipo_aclaracion not in(2,3)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion==0){//si se cargo por aclaraciones finales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa_".$id_evaluacion_enviada_1]."' where evaluador1_id=$id_evaluacion_enviada_1 and pv_id=$pv_id and tipo_aclaracion not in(2,3)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion!=0){//si se cargo por aclaraciones finales
								$cambia_observacion = "update $t32 set observacion = '".$_POST["observa1_".$id_evaluacion_enviada_1]."' where evaluador1_id=$id_evaluacion_enviada_1 and pv_id=$pv_id and tipo_aclaracion in(2,3)";
							}
					$sql_ob=mysql_query($cambia_observacion);
						...
						if($tipo_aclaracion_aprobacion==0){//si se cargo por ofertas normales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada_1,$pv_id,'".$_POST["observa_".$id_evaluacion_enviada_1]."', 0)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion==0){//si se cargo por aclaraciones finales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada_1,$pv_id,'".$_POST["observa_".$id_evaluacion_enviada_1]."', 0)";
							}elseif($tipo_aclaracion_aprobacion!=0 and $tipo_aclaracion!=0){//si se cargo por aclaraciones finales
								$cambia_observacion = "insert into $t32 (evaluador1_id, pv_id, observacion, tipo_aclaracion) values ($id_evaluacion_enviada_1,$pv_id,'".$_POST["observa1_".$id_evaluacion_enviada_1]."', 3)";
							}
			...
			//echo $id_evaluacion_enviada." ".$participacion_criterio." ".$valor_obtenido."<br>";
				if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$busca_exite = traer_fila_row(query_db("select * from $t96  where evaluador1_id = $id_criterio and pv_id = $pv_id and (tipo_aclaracion not in(2,3) or tipo_aclaracion is null)"));
				}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
				$busca_exite = traer_fila_row(query_db("select * from $t96  where evaluador1_id = $id_criterio and pv_id = $pv_id and tipo_aclaracion in(2,3)"));
				}
				if($busca_exite[0]>=1){
					
					if($tipo_aclaracion==0){//si se cargo por ofertas normales
					$query="update $t96 set resultado_evaluacion = $valor_obtenido , resultado_evaluacion_afectado = $participacion_criterio where evaluador1_id = $id_criterio and pv_id = $pv_id and (tipo_aclaracion not in(2,3) or tipo_aclaracion is null)";
						$cambia_resultado = "update $t96 set resultado_evaluacion = $valor_obtenido , resultado_evaluacion_afectado = '$participacion_criterio' where evaluador1_id = $id_criterio and pv_id = $pv_id and (tipo_aclaracion not in(2,3) or tipo_aclaracion is null)";
					}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
					$query="update $t96 set resultado_evaluacion = $valor_obtenido , resultado_evaluacion_afectado = $participacion_criterio where evaluador1_id = $id_criterio and pv_id = $pv_id and tipo_aclaracion in(2,3)";
						$cambia_resultado = "update $t96 set resultado_evaluacion = $valor_obtenido , resultado_evaluacion_afectado = '$participacion_criterio' where evaluador1_id = $id_criterio and pv_id = $pv_id and tipo_aclaracion in(2,3)";
					}
					...
					{
					if($tipo_aclaracion==0){//si se cargo por ofertas normales
						$insertar = query_db("insert into $t96 (pv_id, evaluador1_id, evaluador6_nombre, evaluador6_tamano, evaluador6_tipo, evaluador6_observaciones, evaluador6_fecha, resultado_evaluacion, resultado_evaluacion_afectado, tipo_aclaracion) values ($pv_id,$id_criterio, '',0,'','','','$valor_obtenido', '$participacion_criterio' 0)");
					}elseif($tipo_aclaracion!=0){//si se cargo por aclaraciones finales
						$insertar = query_db("insert into $t96 (pv_id, evaluador1_id, evaluador6_nombre, evaluador6_tamano, evaluador6_tipo, evaluador6_observaciones, evaluador6_fecha, resultado_evaluacion, resultado_evaluacion_afectado, tipo_aclaracion) values ($pv_id,$id_criterio, '',0,'','','','$valor_obtenido', '$participacion_criterio' 3)");
					}
				...
				?>
				<script>alert('<?=$tipo_aclaracion_aprobacion;?>')</script>
				<?
			if($tipo_aclaracion_aprobacion!=0){
				if($tipo_evaluacion!=25){
					foreach($_POST["evaluacion_tecnica1"] as $id_evaluacion_enviada3 => $valor_obtenido2){
						if($valor_obtenido2=="Sin"){
							$estado_p+=1;
						}
					}
...
$duplica_documentos = mysql_query("select * from $t6 where pro1_id = $id_invitacion_pasa");
		...
		elseif($b==7)
			{
			$cambia_estado_origen=query_db("update $t5 set tp1_id = $b where pro1_id = $id_invitacion_pasa ");
			auditor(60,$id_invitacion_pasa,$comentarios_no_adjudica, "");			
			...
			$id_subastas_arrglo = str_replace('---detalle_contratar---', $sql_e[12], $id_subastas_arrglo);
				...
				$graba_correo_pro2="";
					...
					$busca_provee_contactos = query_db("select distinct email, contacto from v_relacion_contactos_procesos where pro1_id = $id_invitacion_pasa and pv_id =$lp[0]");
						...
						auditor(59,$id_invitacion_pasa,$lp[2]." | Se envio email de ".listas_sin_select($tp1,$sql_e[1],1).", e-mail notificados: ".$graba_correo_pro2, "");
						}// provvedores
		...
		window.parent.ajax_carga('historico-proceso_0.html','contenidos');
			...
			if($b==8) $accion_adju=61;
			elseif($b==11) $accion_adju=62;
			
			auditor($accion_adju,$id_invitacion_pasa,$comentarios_no_adjudica, "");
				$cambia_estado_origen=query_db("update $t5 set tp1_id = $b where pro1_id = $id_invitacion_pasa ");
				$sql_ex_no_ad = "insert into $t46 (pro1_id, pro27_id, pv_id, tipo_adj_no_adj, us_id, fecha_envio, notificado, observacion_admin) 
					values ($id_invitacion_pasa,0,0,4,".$_SESSION["id_us_session"].",'$fecha $hora', 1, '".$comentarios_no_adjudica."')";
					$sql_ex_no_ad1=query_db($sql_ex_no_ad);
...
if($accion=="crea_pregunta_aclaracion_final")
	{
		...
		$cambia = query_db("insert into  $t27 (pro1_id ,pv_id ,us_id, fecha_solicitud,fecha_limite_respuesta, objeto, detalle ,leida,anexo, tipo) 
		values ($id_invitacion_ar, '".$id_pro."',".$_SESSION["id_us_session"].",'$fecha $hora', '$h_m_r','".elimina_comillas_2($asunto_cartelera)."','".elimina_comillas_2($pregunta_general)."',1,'".$_FILES["anexo_re_general"]["name"]."', $tipo_cartelera )");
		$id_p_archivo = id_insert();
		
		if($_FILES["anexo_re_general"]["name"]!=""){			
			carga_archivo($_FILES["anexo_re_general"]["tmp_name"],"procesos_cartelera/c_a_f_a_".$id_p_archivo);	
		}
		if($tipo_cartelera==2 or $tipo_cartelera==3){//si se vuelve a abrir las ofertas tecnicas o economicas por aclraciones finales
			$inserta_procesos="update evaluador11_proveedores_con_oferta_tec set estado = 1 where pro1_id=$id_invitacion_ar";
		$sql_e = query_db($inserta_procesos);
			?>
    	<script>
...
<?
	...
	?>
    	<script>
				...
				forma.target="grp_urna";
					forma.action = "../../librerias/php/funcion_urna_sgpa.php";
					forma.accion.value="graba_gestion_apertura_devuolucion_pendi";
					forma.contiene_tecnico.value="1"
					forma.submit()
					forma.target="";
					forma.action = "";
