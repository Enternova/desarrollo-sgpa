...
if($_POST["accion"]=="crea_adjudicacion_proverdor")
	{
		...
		if($_POST["documento_".$pv_id]=="0")
			$msg .= " * Seleccione el tipo de adjudicación (Total / Parcial) ";
	...
	$sql_e_invitacion=traer_fila_row(query_db($busca_procesos));
		...
		echo  $inserta_procesos="insert into $t43 (pro1_id,pv_id, fecha, us_id, documento, fecha_entrega,contacto, pro25_id, estado,acepta_terminos,us_id_aceptacion,fecha_aceptacion,ip_aceptacion,observacion_no_acepta,cargo_contable, nuemro_aprobacion)
		...
		$busca_carta = traer_fila_row(query_db("select * from $tp12 where tp12_id = 14"));
		$datos_bicacion = traer_fila_row(query_db("select * from $t41 where pro25_id = ".$_POST["pro25_id_".$pv_id]));
			$id_subastas_arrglo = str_replace("---proceso---",$sql_e_invitacion[22], $busca_carta[4] );
			$id_subastas_arrglo = str_replace('---proveedor---',$_POST["nombre_provee_".$pv_id], $id_subastas_arrglo);
			$id_subastas_arrglo = str_replace('---pedido--',$sql_e_invitacion[12], $id_subastas_arrglo);
			$id_subastas_arrglo = str_replace('---tipo_adjudicacion---',$_POST["documento_".$pv_id], $id_subastas_arrglo);
		...
		window.parent.ajax_carga('../aplicaciones/evaluacion/adjudicacion_paso1_servicios.php?id_invitacion=<?=$id_invitacion;?>','contenidos');
		</script>
        <?
		}
		else{
...
if($_POST["accion"]=="ed_adjudicacion_proverdor")
	{
		...
		$datos_bicacion = traer_fila_row(query_db("select * from $t41 where pro25_id = ".$_POST["pro25_id_".$pv_id]));
			...
			$id_subastas_arrglo = str_replace('---sitio_entrega---',$datos_bicacion[2] , $id_subastas_arrglo);
			$id_subastas_arrglo = str_replace('---direccion_entrega---', $datos_bicacion[3] , $id_subastas_arrglo);
