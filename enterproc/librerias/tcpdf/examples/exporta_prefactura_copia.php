...
$text.='<table width="100%" border="0" cellpadding="2" cellspacing="2" >
  ...
  <tr>
   ...
   <tr>
    <td style="font-size:30px; "><div align="left"><strong>El contrato es tipo:</strong>  '; if($sql_busca_temporal_pre_fa[15]==1){ $text.='Marco | orden de trabajo:'.$sql_busca_temporal_pre_fa[16]; } else $text.='Normal';
	$text.='</div></td>
  </tr> 
  <tr>
...
<br />';			
 ...
 $text.='<table width="100%" border="0" cellspacing="2" cellpadding="2">
            <tr>
              <td style="font-size:34px; border-bottom-color:#C7422F 2px;"><strong>PROYECTO: '.htmlentities(strtoupper($lista_proyectos_pre[4])).'</strong></td>
            </tr>
          </table>';
		$categoria_imprime="";		
   ...
   $text.='  <table width="100%" border="0" cellspacing="4" cellpadding="4">';
        ...
        $categoria_imprime='<spam style="font-size:30px;">Categoria: '.elimina_comillas_2_inv($lista_categoria[0]).'</spam>';
            } 
		...
		while($lista_grupos=traer_fila_row($sqlgrupo)){//grupos
            ...
            if(chop($lista_grupos[0])<>""){ 
		           $grupos_imprime=' <spam style="font-size:30px; ">Grupo: '.elimina_comillas_2_inv($lista_grupos[0]).'</spam>';
            } 
...
$crea_campo_columna_sin="";	
	...
	$detall_m_no_sel="";		
      ...
      ';
            ...
            $text.=' <table width="100%" border="0" cellspacing="2" cellpadding="2">
			<tr>
              <td  colspan="6" style=" font-size:20px; border-top: 1px dashed #ff9999; border-bottom: 1px dashed #ff9999; " align="CENTER">FINAL DE TARIFAS PARA EL PROYECTO: '.htmlentities($lista_proyectos_pre[4]).'</td>
...
$pdf->Output('Tiquete_servicio_'.$consecutivo_proceso.'pdf', 'I');
