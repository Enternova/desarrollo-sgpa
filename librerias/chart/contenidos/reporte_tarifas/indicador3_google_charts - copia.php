...
while($sel_pro = traer_fila_db($selec_t_pro)){
	...
	$sql_barra_2 = traer_fila_row(query_db("select count(distinct  id_contrato_tarifas) from v_reporte_general_variacion_tarifas where id_gerente = ".$sel_pro[0]." and t6_tarifas_estados_contratos_id = 6"));
			...
			$.post('indicador1_sub2_google_charts.php', {categoria: text, usadas: col, tp_grafica: 2}, function(data6) {//se pasa por post la variable para esperar la respuesta con los datos para armar la otra gráfica
				...
				for(var i=0; i<option.length-1; i++){
					...
					if(parseInt(num_col[0])==0){
						...
						data9.addRow([""+row[0], parseInt(row[1]), parseInt(row[2]),parseInt(row[3])])
					}
				}
			      var options = {
				colors: ['#99C754', '#54C7C5'],
...
<body>
	<div class="font" style="font-size:16pt !important; font-weight: 900 !important;"><br><br>No se puede mostrar la gr&aacute;fica porque debe seleccionar m&aacute;ximo 3 &aacute;reas usuarias</div>
</body>
</html>
<?}
?>