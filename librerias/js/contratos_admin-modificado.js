...
function devolver_anterior(fecha_ini, ob,id_campo, fecha_ini_campo, ob_campo, rol1, rol2, ob_rol2, ob_campo_rol2){
	var forma = document.principal
...
function graba_fecha_leg(fecha_ini, fecha_fin, ob, fecha_ini_campo, fecha_fin_campo, ob_campo, ob_obligatorio, ob_rol2, ob_campo_rol2, rol1, rol2, edita_fecha_2, id_actividad){
	...
	var alerta = confirm("Esta seguro de grabar esta fecha?")
		if(alerta){
			forma.action = "procesos-contratos.html";
...
function graba_informacion_contrato(graba_aseguramiento_admin){
	...
	if(msg!=""){
		...
		muestra_alerta_error_solo_texto('', 'Error', 'Por favor verificar los siguientes campos:<br>'+msg, 60, 5, 10)
		return
	}else{
...
function graba_informacion_poliza2(){
	...
	if(forma.observaciones.value=="0"){
...
function elimina_poliza(id_poliza){
		muestra_alerta_general_solo_texto("elimina_poliza_continua('"+id_poliza+"')", "Advertencia", "¿Esta seguro de Eliminar esta Poliza?", 20, 10, 18)	
}	
function elimina_poliza_continua(id_poliza){
	var forma = document.principal
	forma.action = "procesos-contratos.html";
