...
function aprueba_firma_adjudica(id_rol, tipo_firma, conflicto_interes, observacion, alerta_vencimiento,activaalerta) {
	...
	if (tipo_firma != 1 && observacion =="") {
       var_tex = "* Si no se va a firmar la solicitud es necesario que digite la observacion";
		//return;
    }
	...
	if(tipo_firma==1 && conflicto_interes==1){
		var_tex = "* No puede firmar si tiene conflicto de intereses, lo que debe hacer es devolver el punto y describir el conflicto en la observacion";
		//return;
	}
...
function elimina_proveedor(id_proveedor,tipo_elimina,vez) {
    var forma = document.principal
