...
function graba_valida_modificacion_manual(aprobacion, ob, aprobador){
	var forma = document.principal
...
if (valida_texto_espacios(forma.ob_reemplazo.value) == "NO"  || characterCount(forma.ob_reemplazo.value,20) != "") {
        msg = msg + "* Digite la observacion del reemplazo.\n"
...
function valida_tipo_doc(id_item_pecc, tipo) {
    ...
    if (tipo == 2) {//marco
        ajax_carga('../aplicaciones/pecc/adjudicacion-marco.php?id_item_pecc=' + id_item_pecc + '&id_tipo_proceso_pecc=' + forma.id_tipo_proceso_pecc.value + '&tipo_documento=2', 'contenidos')
    }
    if (tipo == 1) {
        ajax_carga('../aplicaciones/pecc/adjudicacion.php?id_item_pecc=' + id_item_pecc + '&id_tipo_proceso_pecc=' + forma.id_tipo_proceso_pecc.value + '&tipo_documento=1', 'contenidos')
    }
    if (tipo == 4) {
        ajax_carga('../aplicaciones/pecc/adjudicacion-desierto.php?id_item_pecc=' + id_item_pecc + '&id_tipo_proceso_pecc=' + forma.id_tipo_proceso_pecc.value + '&tipo_documento=4', 'contenidos')
    }
...
/*
    ...
    if (valida_texto_espacios(forma.alcance.value) == "NO"  || characterCount(forma.alcance.value,20) != "") {
    ...
    if (valida_texto_espacios(forma.justificacion2.value) == "NO"  || characterCount(forma.justificacion2.value,20) != "") {
        msg = msg + "* Digite la Justificación Tecnica\n"
    ...
    if (valida_texto_espacios(forma.recomendacion.value) == "NO"  || characterCount(forma.recomendacion.value,20) != "") {
	 ...
	 if (valida_texto_espacios(forma.campos1.value) == "NO"  || characterCount(forma.campos1.value,20) != "") {
        msg = msg + "* Digite la Oportunidad\n"
        forma.campos1.className = "textarea_faltantes";
    } else {
        forma.campos1.className = "";
    }
	
	if (valida_texto_espacios(forma.campos2.value) == "NO"  || characterCount(forma.campos2.value,20) != "") {
        msg = msg + "* Digite el Costo Beneficio\n"
        forma.campos2.className = "textarea_faltantes";
    } else {
        forma.campos2.className = "";
    }
	...
	if (valida_texto_espacios(forma.campos4.value) == "NO"  || characterCount(forma.campos4.value,20) != "") {
        msg = msg + "* Digite la Optimizar Transferencia Riesgos\n"
	...
	if (valida_texto_espacios(forma.campos5.value) == "NO"  || characterCount(forma.campos5.value,20) != "") {
        msg = msg + "* Digite la Trazabilidad \n"
	...
	if (valida_texto_espacios(forma.campos6.value) == "NO"  || characterCount(forma.campos6.value,20) != "") {
        msg = msg + "* Digite la Transparencia \n"
	...
	if (valida_texto_espacios(forma.campos7.value) == "NO"  || characterCount(forma.campos7.value,20) != "") {
        msg = msg + "* Digite la Sostenibilidad \n"
...
function acciones_administrativas_profesionales() {
    ...
    if (forma.usuario_permiso.value == "") {
        alert("Debe Seleccionar el Gerente del Contrato");
        return;
    }
    if (forma.acci5.value == 1 && (forma.ob5.value == "" || forma.ob5.value == " ")) {
        alert("Para congelar un proceso debe digitar una observacion");
...
function acciones_administrativas() {
    ...
    if (forma.acci6.value == 1) {
        mensaje = "\n * Va a Eliminar este proceso de los Historicos";
        ...
        if (forma.ob6.value == "" || forma.ob6.value == " ") {
            alert("Para Eliminar un proceso debe digitar una observacion");
            return;
        }
    ...
    if (forma.acci8.value == 1) {
        ...
        if (forma.adjunto_para_eliminar_urna.value == "") {
            alert("Debe Seleccionar un archivo adjunto para poder eliminar");
            return;
        }
        if (forma.ob8.value == "" || forma.ob6.value == " ") {
            alert("Para Eliminar una urna debe digitar una observacion");
            return;
        }
...
function valida_tipo_proceso(tipo_proceso) {
	...
	if(tipo_pro == 4 || tipo_pro == 5) {
		...
		document.getElementById("muestra_oportunidad").style.display = "none"
		document.getElementById("muestra_calidad").style.display = "none"
...
function cambia_titulo_valores(valor) {
    if (valor == 1) {
        document.getElementById("titulo_valores").innerHTML = 'Distribuci&oacute;n del Valor a Solicitar'
...
function aprueba_firma_adjudica(id_rol, tipo_firma, conflicto_interes, observacion, alerta_vencimiento) {
    var forma = document.principal
	...
	if(conflicto_interes==0){
		alert(" Por favor seleccione si tiene o no conflicto de intereses");
		return;
	}
	
	if (tipo_firma != 1 && observacion =="") {
       alert("Si no se va a firmar la solicitud es necesario que digite la observacion");
		return;
    }
	...
	if(tipo_firma==1 && conflicto_interes==1){
		alert(" No puede firmar si tiene conflicto de intereses, lo que debe hacer es devolver el punto y describir el conflicto en la observacion");
		return;
	}
...
function aprueba_firma(id_rol, tipo_firma, conflicto_interes, observacion) {
    var forma = document.principal
...
if(mensaje == "Esta Seguro de firmar y declarar que no tiene conflicto de intereses?"){
if (forma.conflito_intere_sel.value == "0") {
            alert( " Seleccione si tiene conflicto de intereses\n")
            forma.conflito_intere_sel.className = "select_faltantes";
			return;
        } else if(forma.conflito_intere_sel.value == "1"){
			alert( " Si tiene conflicto de intereses no puede poner en firme la solicitud\n")
            forma.conflito_intere_sel.className = "select_faltantes";
			return
			}
		else{		
...
function devolver_item_a_gerente_contrato() {
    var forma = document.principal

    if (forma.observa_atras.value == "") {
        alert("Por favor digite una observacion");
        return;
    }
    var alerta = confirm("Esta seguro de Devolverle esta Solicitud al Gerente del Contrato?")
...
function agrega_proveedor_ser_menor(tipo, id_proveedor_relacionado) {
    var forma = document.principal
    var msg = ""
...
function valida_proveedor_nuevo_base_datos() {
    ...
    if (forma.proveedores_busca.value == "") {
        msg = msg + "* Digite el Nombre ó Razon Social del Proveedor\n"
        forma.proveedores_busca.className = "select_faltantes";
    } else {
...
function valida_proveedor_nuevo() {
    var forma = document.principal
    var msg = ""
    ...
    if (forma.nom.value == "") {
        msg = msg + "* Digite el Nombre ó Razon Social del Proveedor\n"
        forma.nom.className = "select_faltantes";
    } else {
        forma.nom.className = "";
    }
...
function valida_graba_item_edita(tipo) {
    ...
    if (tipo == 2 || tipo == 3) {
		...
		if (forma.tipo_proceso.value == 16 && tipo == 3) {
                  if (valida_texto_espacios(forma.justificacion.value) == "NO"  || characterCount(forma.justificacion.value,20) != "" ) {
					msg = msg + "*  Digite la Justificacion Comercial\n" + characterCount(forma.justificacion.value,20);
					forma.justificacion.className = "textarea_faltantes";
				} else {
					forma.justificacion.className = "";
				}
            ...
            if (forma.tipo_proceso.value == 11) {
                if ((forma.contratos_normales.value == "" || forma.contratos_normales.value == "-C- Contratista: ----,----,") && (forma.solicitud_que_carga.value == "" || forma.solicitud_que_carga.value == 0)) {
                    alert("Va a crear la solicitud sin relacionar contrato ni solicitud")
                }
            }
...
/*FIN OBJETIVOS DEL PROCESO*/
			...
			if(forma.estado_actual_del_proceso.value != 31){
				 if (forma.tipo_proceso.value == 1 || forma.tipo_proceso.value == 2 || forma.tipo_proceso.value == 6 || forma.tipo_proceso.value == 3) {
					if (forma.req_contra_mano_obra_local.value == "0") {
						msg = msg + "* ATENCION: Seleccione si requiere contratacion de mano de obra local\n"
						forma.req_contra_mano_obra_local.className = "select_faltantes";
					}else{		
						forma.req_contra_mano_obra_local.className = "";	
					} 
					if (forma.req_cont_bien_ser_local.value == "0") {
						msg = msg + "* ATENCION: Seleccione si requiere contratacion de bienes o servicios local\n"
						forma.req_cont_bien_ser_local.className = "select_faltantes";
					}else{		
						forma.req_cont_bien_ser_local.className = "";	
					} 
    ...
    */}
	...
	if (forma.tipo_proceso.value == 12) {
        ...
        if (forma.valor_usd.value > 0) {
            msg = msg + "* Para solicitues tipo (Reclasificacion) el valor USD debe ser 0 (cero)\n"
            forma.valor_usd.className = "campos_faltantes";
        }
        else {
            forma.valor_usd.className = "";
		...
		if (forma.valor_cop.value > 0) {
            msg = msg + "* Para solicitues tipo (Reclasificacion) el valor COP debe ser 0 (cero)\n"
