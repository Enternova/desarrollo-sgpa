...
function envia_email_polizas_completas($id_contrato_llega){
	...
	$consecutivo_imp = numero_item_pecc_contrato($numero_contrato1,$numero_contrato2,$numero_contrato3, $numero_contrato4);
...
function envia_email_contrato_legalizacion($id_contrato_llega,$id_complemento){
	...
	if($id_complemento>0){
		$alcance = "";
		$bus_complemento = "select t7c.id,t7c.id_item_pecc,t7c.numero_otrosi,t1t.nombre,t7c.gerente,t7c.alcance from $co4 t7c left join $g8 t1t on t7c.tipo_complemento = t1t.id where t7c.id=$id_complemento";
	...
	$sel_permisos_email2 = query_db("select t1u.us_id,t1u.email from ".$g1." t1u where t1u.us_id=".$sql_con_com[16]);	
	$correo_destino2 = "";
	$coma = "";
	...
	//Envio_email
		...
		$mail->AddAddress("Lissette.patino@aon.com");
		...
		$mail->AddAddress("eliana.avila@hcl.com.co");
...
function envia_email_polizas_vence(){
	...
	$sel_permisos_email = query_db("select tsup.id_usuario,t1u.email from ".$g1." t1u left join ".$ts5." tsup on t1u.us_id=tsup.id_usuario  where tsup.id_permiso=12");
