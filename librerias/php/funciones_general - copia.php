...
$id_log = log_de_procesos_sgpa(1, 18, 0, $id_item_pecc, $sel_item[14], 20);//agrega valores
	...
	$sel_contratos_sol_original = traer_fila_row(query_db("select * from $co1 where t1_proveedor_id='".$sel_contras[1]."' and apellido='".$sel_contras[4]."' where id_item = ".$sel_item[43]));
	
	if($sel_item[69]==1 and $sel_contratos_sol_original[0]>0){//*************************si es una modificacion de adjudicacion***********************
			$updta = query_db("update $co1 set id_item = ".$id_item_pecc.", objeto='".$sel_item[9]."', monto_usd=".$sel_contras[2].", monto_cop=".$sel_contras[3].", gerente=".$sel_item[3]." where id = ".$sel_contratos_sol_original[0]);
