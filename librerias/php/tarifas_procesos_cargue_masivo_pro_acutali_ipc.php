...
$sel_contrato_modulo_contratos = traer_fila_row(query_db("select id_contrato from t6_tarifas_contratos where tarifas_contrato_id = ".$id_contrato_arr));
		...
		if( ($sql_busca_tarifas_gerente[2]==2 ) && ($_POST["aprobacion_secundaria_0"]=="0") ){	
				?>
				<script>window.parent.muestra_alerta_error_solo_texto('', 'Error', 'Seleccione la persona que solicita la actualización', 20, 10, 18);
	...
	$valor_excel_anetrior=numerico($data->sheets[0]['cells'][$i][9]);
		...
		if($_POST["modi_convencion_0"]=="1")
			...
			if($_POST["modi_convencion_0"]=="2")
				$tipo_modificacion_normal_convencion = 4;
...
/*********************VALIDA QUE LE ARHIVO CONINCIDA***********************************/
			...
			$detalle=elimina_comillas_carge_2($data->sheets[0]['cells'][$i][15]);
			if($detalle=="")
				$err_m = "* Las observaciones de la fila ".$i." no tiene contenido";


			$valor=numerico($data->sheets[0]['cells'][$i][13]);
			if($valor==2)
				$err_m = "* El procentaje de IPC de la fila ".$i." esta errado  * El valor no puede cero * El valor no debe llevar formatos numero * El valor no debe llevar formatos de %";

			
			$columna_valor_nuevo = $data->sheets[0]['cells'][$i][13];
			...
			if( ($valida_valor<=0) || ($valida_valor>10) )
				$err_m = "* El procentaje de IPC de la fila ".$i." esta errado  * El valor no puede cero * no puede ser mayor a 10";
			...
			$fecha_inicio_vigencia_1 = $data->sheets[0]['cells'][$i][14];
			...
			if($fecha_inicio_vigencia_valida==2)
				$err_m = "* La fecha de la fila ".$i." esta errada el formato debe ser dd/mm/yyyy !".$fecha_inicio_vigencia_valida;
...
/******VALIDA ERRORES**********************/			
	...
	$sql_ex_lis_existe = traer_fila_row(query_db($busca_tarifas_contrato));
			...
			$codigo_proveedor=$sql_ex_lis_existe[4];
			$detalle=$sql_ex_lis_existe[5];
			$unidad_medida=$sql_ex_lis_existe[6];
			$cantidad=$sql_ex_lis_existe[7];
			$t1_moneda_id=$sql_ex_lis_existe[8];
			$fecha_fin_vigencia=$sql_ex_lis_existe[16];
			$valor_ipc=$data->sheets[0]['cells'][$i][13]; // porcetaje
			...
			$tipo_creacion=2;
			$t6_tarifas_estados_tarifas_id=9;//estado provicional
			...
			if($fecha_inicio_vigencia_1!="")			
				$fecha_inicio_vigencia=cambia_formato_fecha($data->sheets[0]['cells'][$i][14]);//suma_dia_fecha_a(1,cambia_formato_fecha($data->sheets[0]['cells'][$i][16]));
			else
			...
			$detalle_descuento_pasa=0;
			$detalle_observaciones=elimina_comillas_carge_2($data->sheets[0]['cells'][$i][15]);
			...
			$valor = ($sql_ex_lis_existe[9]+($sql_ex_lis_existe[9]*$valor_ipc)/100);
...
echo $insert_fila;
		...
		$sql_ex=query_db($insert_fila.$trae_id_insrte);
			...
			?>
                    ...
                    window.parent.ajax_carga('../aplicaciones/tarifas/proveedor/c_tarifas_actualicar_masivas_ipc.php?id_contrato=<?=$id_contrato;?>&lista_existentes=<?=$id_lista;?>','carga_acciones_permitidas');
                    </script>
			<?
