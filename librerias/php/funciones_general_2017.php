...
if($pecc == "PECC"){
	...
	$insert = "insert into $pi14 (id_item_pecc, id_rol, orden, estado,por_sistema, tipo_adj_permiso) values (".$id_item_pecc.", 51,6,1,1,".$tipo_adj_permiso.")";
	$sql_ex=query_db($insert.$trae_id_insrte);
	$id_ingreso = id_insert($sql_ex);	
	...
	/*sies un otrosi o una ampliacion incluya la firma de VP financiero*/
	if($sel_item[6] == 5 || $sel_item[6] == 7 ){//si es reclasificacion
	$insert = "insert into $pi14 (id_item_pecc, id_rol, orden, estado,por_sistema, tipo_adj_permiso) values (".$id_item_pecc.", 20,10,1,1,2)";
	$sql_ex=query_db($insert.$trae_id_insrte);
	$id_ingreso_vp_finanzas = id_insert($sql_ex);

	$insert52 = query_db("insert into $pi15 (id_secuencia_solicitud,id_usuario, estado, id_usuario_original) values (".$id_ingreso_vp_finanzas.", '".cual_es_el_reemplazo($id_vp_finanzas)."',1, '".$id_vp_finanzas."')");
	}
