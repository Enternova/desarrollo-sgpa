...
function acciones_administrativas_profesionales() {
    ...
    if (forma.acci5.value == 1 && (forma.ob5.value == "" || forma.ob5.value == " ")) {
        mensaje = "* Para congelar un proceso debe digitar una observacion";
        //return;
    }
...
function aprueba_firma_adjudica(id_rol, tipo_firma, conflicto_interes, observacion, alerta_vencimiento,activaalerta) {
	...
	if(activaalerta == ""){
		if(tipo_firma == 1){//es para aprobar
			window.parent.muestra_alerta_general_desde_select('aprueba_firma_adjudica(-comillas-'+id_rol+'-comillas-, -comillas-'+tipo_firma+'-comillas-, -comillas-'+conflicto_interes+'-comillas-, -comillas-'+observacion+'-comillas-, -comillas-'+alerta_vencimiento+'-comillas-,1)', 'Advertencia','* Esta Seguro de Firmar esta Solicitud', 40, 5, 12)
			return;
		}
		if(tipo_firma == 2){//es para devolver
			window.parent.muestra_alerta_general_desde_select('aprueba_firma_adjudica(-comillas-'+id_rol+'-comillas-, -comillas-'+tipo_firma+'-comillas-, -comillas-'+conflicto_interes+'-comillas-, -comillas-'+observacion+'-comillas-, -comillas-'+alerta_vencimiento+'-comillas-,1)', 'Advertencia','* Esta Seguro de Devolver al Profesiona de Abastecimiento / Comprador', 40, 5, 12)
			return;
		}
		if(tipo_firma == 3){//es para rechazar
			window.parent.muestra_alerta_general_desde_select('aprueba_firma_adjudica(-comillas-'+id_rol+'-comillas-, -comillas-'+tipo_firma+'-comillas-, -comillas-'+conflicto_interes+'-comillas-, -comillas-'+observacion+'-comillas-, -comillas-'+alerta_vencimiento+'-comillas-,1)', 'Advertencia','* Esta Seguro de Rechazar la Solicitud', 40, 5, 12)
			return;
		}
		}
...
function aprueba_firma(id_rol, tipo_firma, conflicto_interes, observacion,activaalerta) {
	...
	if(activaalerta == ""){
		...
		if(tipo_firma == 2){//es para devolver
			window.parent.muestra_alerta_general_desde_select('aprueba_firma(-comillas-'+id_rol+'-comillas-, -comillas-'+tipo_firma+'-comillas-, -comillas-'+conflicto_interes+'-comillas-, -comillas-'+observacion+'-comillas-,1)', 'Advertencia','* Esta Seguro de Devolver al Profesiona de Abastecimiento / Comprador', 40, 5, 12)
			return;
		}
		if(tipo_firma == 3){//es para rechazar
			window.parent.muestra_alerta_general_desde_select('aprueba_firma(-comillas-'+id_rol+'-comillas-, -comillas-'+tipo_firma+'-comillas-, -comillas-'+conflicto_interes+'-comillas-, -comillas-'+observacion+'-comillas-,1)', 'Advertencia','* Esta Seguro de Rechazar la Solicitud', 40, 5, 12)
			return;
		}
		}
    //var alerta = confirm(var_tex)
...
if(mensaje == "Esta Seguro de firmar y declarar que no tiene conflicto de intereses * Declaro que he revisado la lista de conflicto de interés suministrada por Cumplimiento en la cual no se registra conflicto de ninguno de los participantes en éste proceso.?"){
