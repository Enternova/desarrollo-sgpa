...
if($sel_item[6] == 7 or $sel_item[6] == 8 or $sel_item[6] == 4 or $sel_item[6] == 5 or $sel_item[6] == 9 or $sel_item[6] == 10 or $sel_item[6] == 11 or $sel_item[6] == 12){
	...
	if($sel_item[14] == 6){	
		...
		echo "<br />Actualiza con Funcion<br />";
...
$id_log = log_de_procesos_sgpa(1, 18, 0, $id_item_pecc, $sel_item[14], 20);//agrega valores
	...
	$sele_presu = query_db("select $pi8.t2_presupuesto_id, $pi18.id_relacion from $pi8 as t1, $pi18 as t2 where t1.t2_item_pecc_id = $id_item_pecc and t1.permiso_o_adjudica = 2 and t1.t2_presupuesto_id = t2.t2_presupuesto_id and t1_proveedor_id=".$sel_contras[0]." and t2.apellido = '".$sel_contras[4]."'");
...
function crea_antecedente_auto($id_item_pecc, $tipo, $consecutivo){
		...
		/*if( $tipo == "crea_urna"){	$detalle = "Se Creo en la Urna Virtual - $fecha";	
			
		
		}*/
/*		if( $tipo == "crea_otro_si"){ $detalle = "Se Creo el Otro Si No. $numero_otto - $fecha";			}
		if( $tipo == "crea_ot"){	$detalle = "Se Creo la Orden de Trabajo No. $numero_ot  - $fecha";		}
		if( $tipo == "amplia_soli"){	$detalle = "Se Amplio la Solictud  - $fecha";		}
		if( $tipo == "crea_contrato"){	$detalle = "Se Adjudico Contrato No. $numero_contrato - $fecha";		}
		if( $tipo == "aprueba_permiso"){	$detalle = "Se Aprobo el Permiso - $fecha";		}
*/
...
function crear_en_e_procurement($id_item_pecc){
	...
	$sel_item = traer_fila_row(query_db("select t1_tipo_proceso_id, t1_tipo_contratacion_id, CAST(objeto_solicitud AS text), id_us_profesional_asignado
	...
	$sel_provee = query_db("select id_proveedor from $pi13 where id_item=".$id_item_pecc);//proveedores
...
function verifica_usuario_indicado_solo_si($id_permiso,$id_item){
	...
	$sel_item_permiso = traer_fila_row(query_db("select id_us,id_us_profesional_asignado from $pi2 where id_item=".$id_item));
			if( $sele_permiso[0] > 0 and ($sel_item_permiso[1] == $_SESSION["id_us_session"])){
					$permiso = "SI";
				}
		return $permiso;
...
function verifica_usuario_indicado($id_permiso,$id_item){
	...
	$sel_item_permiso = traer_fila_row(query_db("select id_us,id_us_profesional_asignado, congelado from $pi2 where id_item=".$id_item));
			if( $sele_permiso[0] > 0 and ($sel_item_permiso[1] == $_SESSION["id_us_session"] or $sel_item_permiso[1] == "" and $sel_item_permiso[2] <> 1)){
					$permiso = "SI";
				}
			}else{
				if( $sele_permiso[0] > 0 and $sel_item_permiso[2] <> 1){
					$permiso = "SI";
				}
				}
		return $permiso;
...
if($id_permiso == 31 or $id_permiso == 6 or $id_permiso == 14){
			$sele_permiso = traer_fila_row(query_db("select id_encargado from $vpeec5 where us_id = ".$_SESSION["id_us_session"]." and t2_nivel_servicio_actividad_id = ".$id_permiso));
				
			$sel_item_permiso = traer_fila_row(query_db("select id_us,id_us_profesional_asignado, congelado from $pi2 where id_item=".$id_item));

			if( ($sele_permiso[0] == 1 or $id_permiso == 31) and $sel_item_permiso[0] == $_SESSION["id_us_session"]){
					$permiso = "SI";

				}

			if( $sele_permiso[0] == 2 and ($sel_item_permiso[1] == $_SESSION["id_us_session"] or $sel_item_permiso[1] == "" or $sel_item_permiso[1] == 0)){
					$permiso = "SI";
				}
}
...
function agrega_gestion_pecc_atras($id_item, $actividad, $fecha_real, $dias, $ob){
	...
	$fecha_ini_ini = $fecha_ini;
		...
		if($fecha_ini > $fecha_fin){
			...
			die ('<br /><strong><font color="FF0000">Error</font></strong>!');
			}
	$dias_habiles = 0;
	//RECORRE FECHAS
...
function agrega_gestion_pecc($id_item, $actividad, $fecha_real, $dias){
	...
	if($actividad==13){//si es finalizacion de evaluacion comercial
			...
			$dias = dias_habiles_entre_fechas($fecha_ini,$fecha_fin);
				...
				echo "update $pi17 set estado = 2 where id_item = $id_item and t2_nivel_servicio_actividad_id =".$actividad;
			$desactiva_anteriorer = query_db("update $pi17 set estado = 2 where id_item = $id_item and t2_nivel_servicio_actividad_id =".$actividad);
...
function carga_sub_menu_comite($id_comite){
	...
	<table width="98%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
        ...
        <tr>
          <td onclick="finalizacion_comite();window.parent.document.getElementById('cargando_pecc').style.display = 'block';"><font color="#FF0000" style="cursor:pointer">&gt;&gt; Finalizaci&oacute;n del Comit&eacute;</font></td>
        </tr>
        <?
		}
...
<table width="98%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
	...
	?>
	  ...
	  ?>
        ...
        <?
        if ($sel_item[6] == 8 and $sel_item[14] > 19 and $sel_item[23] == $_SESSION["id_us_session"] and $sel_item[14] <> 31){
			?>
			 <tr>
...
if($sel_item[14] <> 31 and $muestra == "SI" ){
  ...
  <?
  if($sel_item[14] > 1 and $sel_item[14] <> 31 and $sel_item[14] <> 14 and $sel_item[39] == ""){
  ?>
         <tr>
...
//}
		 ...
		 if($_SESSION["id_us_session"] == $sel_item[23] ){
		 ?>
          <tr>
