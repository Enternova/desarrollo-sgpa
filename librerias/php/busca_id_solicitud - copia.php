...
//error_reporting(E_ALL | E_STRICT);
	...
	$sel_contratos_marco ="select distinct top 15 id_item, num1,num2,num3,numero_contrato,fecha_crea_contrato,objeto_solicitud, objeto_contrato,area,alcance,justificacion,recomendacion, apellido, razon_social,  fecha_fin_contrato, id_contrato, CONCAT(num1,CONVERT(VARCHAR(2), num2),'-',CONVERT(VARCHAR(4),(CASE WHEN(LEN(num3))=4 THEN CONCAT('',num3) WHEN(LEN(num3))=3 THEN CONCAT('0',num3) WHEN(LEN(num3))=2 THEN CONCAT('00',num3) WHEN(LEN(num3))=1 THEN CONCAT('000',num3) else '' end))) as consecutivo from $vpeec4 where id_item > 0 and fecha_fin_contrato>='$fecha_hoy' and CONCAT(num1,CONVERT(VARCHAR(2), num2),'-',CONVERT(VARCHAR(4),(CASE WHEN(LEN(num3))=4 THEN CONCAT('',num3) WHEN(LEN(num3))=3 THEN CONCAT('0',num3) WHEN(LEN(num3))=2 THEN CONCAT('00',num3) WHEN(LEN(num3))=1 THEN CONCAT('000',num3) else '' end)))  like '%$q%'";
