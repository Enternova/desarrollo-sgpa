...
function funcion_duplicar_solicitud($tipo, $original, $destino){
	...
	if($tipo==15){//si es modificacion
			...
			if(($s_i_orig[5] == 1 or $s_i_orig[5] == 2) and $s_i_orig[62] <=13){$estado_resultado = 31;  $valor_permiso_ad = "1";}
...
$trae_id_insrte = " select SCOPE_IDENTITY() AS [SCOPE_IDENTITY]";
	...
	while($s_ane = traer_fila_db($sel_afe_ceco)){
		...
		$insert = query_db("insert into t2_relacion_afe_ceco (id_item, id_campo, afe_ceco, adjunto, permiso_adjudica, estado) values (".$destino.", '".$s_ane[3]."', '".$s_ane[4]."', '".$s_ane[5]."', '".$s_ane[6]."', '".$s_ane[7]."')");
		$sql_ex=query_db($insert.$trae_id_insrte);
		$id_ane_destino = id_insert($sql_ex);
...
/*COMPARACION SI ES CONTRATO MARCO*/
		...
		while($sel_pro_ad = traer_fila_db($sel_prove_adjudi)){
			...
			echo $id_pro_ad_destino_marco;
			}
			$sel_aplica_contra = query_db("select t7_contrato_id, t2_proveedor_adjudica from t2_presupuesto_aplica_contrato where t2_presupuesto_id = ".$sel_p[0]." and t2_proveedor_adjudica = ".$sel_pro_ad[0]);
		while($sel_pro_ad = traer_fila_db($sel_aplica_contra)){			
		echo "insert into t2_presupuesto_aplica_contrato (t2_presupuesto_id, t7_contrato_id, t2_proveedor_adjudica) values (".$id_presup_destino.", '".$sel_pro_ad[0]."', '".$id_pro_ad_destino_marco."')";
...
/*selecciona vista*/
 ...
 $sel_valor_inicial_sq = "select $pi2.id_item, num1,num2,num3 from $pi2 inner join $g13 on $g13.t1_tipo_proceso_id = $pi2.t1_tipo_proceso_id where  $pi2.t1_tipo_proceso_id in (9,10,11,15) and (id_solicitud_relacionada in (".$id_solicitud.") or contrato_id in (".$ids_contras.") or id_item_peec_aplica in (".$id_solicitud."))  and $pi2.estado <> 33";
	...
	/*VERIFICA SI HAY OTS A LA SOLICITUD INICIAL DE AÑOS DIFERENTES PARA AGREGARLOS*/

		if($tipo == "ejecucion"){//solo para reporte de ejecucion
		

		 $sql_ots = "select  id_item_ots_aplica, ano, campo, id_campo, contratos  from t2_reporte_marco_temporal_ejecuciones_excel where id_us= '".$_SESSION["id_us_session"]."' and tipo in ('ots')  group by id_item_ots_aplica, ano, campo, id_campo, contratos";
...
$sql_si_tiene_linea = query_db("select count(*) from t2_reporte_marco_temporal_ejecuciones_excel where id_item = ".$id_ampli_recla_inicial." and ano = ".$v_sql_ots[1]." and id_campo = ".$v_sql_ots[3]." and REPLACE(contratos, ' ', '') like '%".$contrato_ot_sin_espa."%' and id_us = ".$_SESSION["id_us_session"]."");
	...
	$sele_ampli_recla_in = traer_fila_row(query_db("select tipo, num_item from t2_reporte_marco_temporal_ejecuciones_excel where id_item = ".$id_ampli_recla_inicial));
					...
					$insert = "insert into t2_reporte_marco_temporal_ejecuciones_excel (id_us, tipo, id_item, contratos, ano, campo, usd, cop, id_campo,t2_presupuesto_id,num_item,contratista) values (".$_SESSION["id_us_session"].", '".$sele_ampli_recla_in[0]."', '".$id_ampli_recla_inicial."', '".$v_sql_ots[4]."','".$v_sql_ots[1]."','".$v_sql_ots[2]."',0,0,'".$v_sql_ots[3]."','0','".$sele_ampli_recla_in[1]."','')";
		...
		/*FIN VERIFICA SI HAY OTS A LA SOLICITUD INICIAL DE AÑOS DIFERENTES PARA AGREGARLOS*/
...
function solo_anos_actual($incluye_actual){
	//solo se utiliza para seleccionar el año del pecc en la creacion
	global $fecha;
	$fecha_explode = explode("-",$fecha);
	$ano_actual = $fecha_explode[0];
