...
$update_sql = query_db("update $co1 set aseguramiento_admin ='".elimina_comillas_2($_POST["aseguramiento_admin"])."', informe_hse = '".elimina_comillas_2($_POST["info_hse"])."', gerente_por_aseguramiento ='".$id_gerente."', gerente ='".$id_gerente."' where id = ".$id_contrato_arr);
		...
		log_agrega_detalle ($id_log, "Aseguramiento Administrativo", elimina_comillas_2($_POST["aseguramiento_admin"]), "t1_tipo_aseguramiento_admin",1);
		log_agrega_detalle ($id_log, "Informe HSE", elimina_comillas_2($_POST["aseguramiento_admin"]), "",1);
		/** INICIO PARA EL DES-017-17*****/		
			...
			$query="select email from $g1 where us_id=$id_gerente";
			$correo_gerente=traer_fila_row(query_db($query));
			$id_contrato_tarifas="select * from $t1 where id_contrato=$lista_contrato[0]";
			$id_reajustes="select * from $ta26 where t6_tarifas_contratos_id=$id_contrato_tarifas[0]";
			$reajustes='';
			if($id_reajustes[5]==1){
				$reajustes=$reajustes."Aplica";
			}else{
				$reajustes=$reajustes."No Aplica";
			}
			$id_reembolsables="select * from $ta22 where t6_tarifas_contratos_id=$id_contrato_tarifas[0]";
			$reembolsables='';
			if($id_reembolsables[5]==1){
				$reembolsables=$reembolsables."Aplica";
			}else{
				$reembolsables=$reembolsables."No Aplica";
			}
