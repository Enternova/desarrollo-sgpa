...
function funcion_duplicar_solicitud($tipo, $original, $destino, $tp_pro_destino){
	global $pi2;
	...
	if($tipo==16){//si es un SM que paso a ser S
			if($tp_pro_destino == 6){
			$estado_resultado = 14; 
			}else{
				$estado_resultado = 6; 
			...
			$valor_permiso_ad = "1";
			$s_i_orig[5]=$tp_pro_destino;
			$s_i_orig[1]=0;
			$update = query_db("update t2_item_pecc set estado = ".$estado_resultado.", es_modificacion = 1, fue_sm = 'SI',id_solicitud_relacionada='".$original."' where id_item = ".$destino);
			}//fin si es modificacion
...
if($s_i_orig[62]>=20 and ($s_i_orig[5] == 1 or $s_i_orig[5] == 2 or $s_i_orig[5] == 3 or $s_i_orig[5] == 6)){//si es una solicitud que genero contrato
	...
	$profesional_seleccionado_funct = selecciona_profesional($s_i_orig[2], $s_i_orig[4], $id_item_pecc, $s_i_orig[3]);
	
	$s_i_orig[16] = $profesional_seleccionado_funct;
...
function legalizaciones_de_contratos($tipo, $id, $edita){
	...
	/**** frin para el des-091 se busca a que tipo de solicitud pertenece servicios/vinenes ***/
		...
		}elseif($res_co_id[2]==1 and($res_co_id[1]==3 or $res_co_id[1]==10 or $res_co_id[1]==11 or $res_co_id[1]==17 or $res_co_id[1]==13)){//para los demás otrosís que requieren soporte descentralizado
