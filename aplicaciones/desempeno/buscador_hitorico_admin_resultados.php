...
$comple_sql="WHERE id_proveedor not in(0)";//" WHERE (id_evaluador=".$_SESSION["id_us_session"]." or id_crea_aspectos= ".$_SESSION["id_us_session"]." or id_jefe=".$_SESSION["id_us_session"].")";
	...
	$sql_rows="select nombre_proveedor, id_proveedor from (select ROW_NUMBER()Over(order by nombre_proveedor ASC) AS RowNum, nombre_proveedor, id_proveedor from dbo.historico_desempeno_resultados() ".$comple_sql." group by nombre_proveedor, id_proveedor) as resultado_paginado WHERE RowNum BETWEEN ".$regis_inicial." AND ".$regis_final;
	$sql_rows_total="SELECT count(*) FROM (select ROW_NUMBER()Over(order by nombre_proveedor ASC) AS RowNum, nombre_proveedor, id_proveedor from dbo.historico_desempeno_resultados() ".$comple_sql." group by nombre_proveedor, id_proveedor) as resultado_paginado";
	...
	$inicio_tabla='';
	$funcion1='muestra_resultado_proveedor|&#xE417;| |color: #229BFF; cursor: pointer !important; background: trasparent;| |Ver';
	$campos=carga_tabla_hmtl_solo_body_resultados($funcion1, '', '', $sql_rows, 1, 'body_historico_resultados', '&#xE838;');
	$footer=carga_paginador_hmtl_solo_body($regis_inicial, $regis_final, $total_registros[0], $posicion_pagina, $pagina, 8, 'foot_historico_resultados', $registros_por_pagina[0], 'paginador_tabla_historico_resultados');
