...
$comple_sql=" WHERE (id_evaluador=".$_SESSION["id_us_session"]." or id_crea_aspectos= ".$_SESSION["id_us_session"]." or id_jefe=".$_SESSION["id_us_session"].")";
$comple_sql2=$comple_sql;
$val_input="";
$val_select="";
if($_GET["caracter1"] and $_GET["caracter1"]!="" and $_GET["caracter1"]!=" " and $_GET["caracter1"]!="0" and $_GET["caracter1"]!=0){
		$val_select=$_GET["caracter1"];
		$comple_sql.=" AND  id_estado_criterio=".elimina_comillas(arreglo_recibe_variables($_GET["caracter1"]));
}
if($_GET["caracter2"] and $_GET["caracter2"]!="" and $_GET["caracter2"]!=" "){
		$val_input=$_GET["caracter2"];
		$comple_sql.=" AND nombre_proveedor LIKE '%".$_GET["caracter2"]."%'" ;
}
$pagina=0;
$posicion_pagina=0;
$val_actual=0;
...
$sql_rows="SELECT estado_evaluacion, nombre_proveedor, numero_documento, periodo_evaluacion, id_evaluacion FROM (SELECT ROW_NUMBER()Over(order by id_evaluacion desc) As RowNum, id_evaluacion, id_estado_criterio, estado_evaluacion, nombre_proveedor, numero_documento, periodo_evaluacion, id_evaluador, id_crea_aspectos, tipo_documento FROM dbo.historico_desempeno() ".$comple_sql.") as resultado_paginado WHERE RowNum BETWEEN ".$regis_inicial." AND ".$regis_final;
$sql_rows_total="SELECT count(*) FROM (SELECT ROW_NUMBER()Over(order by id_evaluacion desc) As RowNum, id_evaluacion, id_estado_criterio, estado_evaluacion, nombre_proveedor, numero_documento, periodo_evaluacion, id_evaluador, id_crea_aspectos, tipo_documento FROM dbo.historico_desempeno() ".$comple_sql2.") as resultado_paginado";
...
$funcion1='muestra_historico_gestion|&#xE154;| |color: #229BFF; cursor: pointer !important; background: trasparent;| |Gestionar';
	...
	//'<td colspan="5">'.$sql_rows.'</td>'      $campos.$footer

$campos=carga_tabla_hmtl_solo_body($funcion1, '', '', $sql_rows, 4, 'body_historico_procesos');
$footer=carga_paginador_hmtl_solo_body($regis_inicial, $regis_final, $total_registros[0], $posicion_pagina, $pagina, 5, 'foot_historico_procesos', $registros_por_pagina[0], 'paginador_tabla_historico');
