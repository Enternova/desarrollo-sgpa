...
<table width="100%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
	...
	<tr>
		<td width="20%" align="right" >Criterio:</td>
	...
	<tr>
      ...
      if($id_pregunta_arr!=""){
		?>
      	<input name="button2" type="button" class="boton_grabar" id="button2" value="Editar Criterio" onclick="graba_pregunta(<?=$id_pregunta_arr;?>)"/>&nbsp;&nbsp;
      <?  
	  }else{
	  ?>
      <input name="button2" type="button" class="boton_grabar" id="button2" value="Crear Criterio" onclick="graba_pregunta(99)"/>&nbsp;&nbsp;
      <?
	  }
	  ?>
      </td>
	...
	<tr class="fondo_4">
	  <td colspan="3" >Listado de Criterios</td>
  </tr>
  <tr class="columna_subtitulo_resultados">
	  <td colspan="2" >Criterios</td>
      <td  >Editar</td>
  </tr>
  <?
	$lista_poliza_int = "select id,id_plantilla,nombre,estado from ".$evf4."  where estado = 1 and id_plantilla = ".elimina_comillas(arreglo_recibe_variables($id_plantilla_hse))." order by id";
	$sql_poliza_int=query_db($lista_poliza_int);
	while($lista_poliza_int=traer_fila_row($sql_poliza_int)){
	?>    

  <tr >
	  <td colspan="2" ><?=$lista_poliza_int[2];?></td>
