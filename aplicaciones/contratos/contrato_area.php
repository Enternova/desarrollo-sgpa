<? include("../../librerias/lib/@session.php"); 
  ...
  $busca_area_pecc=query_db("SELECT DISTINCT id, nombre FROM tabla_contrato_area($sel_id_item[0])");
  //echo "SELECT DISTINCT id, nombre FROM tabla_contrato_area($sel_id_item[0])";
  while($area=traer_fila_row($busca_area_pecc)){//INICIO WHILE BUSCA_AREA_PECC
    $area_default2=$area_default2."'".$area[0]."'";
  }//FIN WHILE BUSCA_AREA_PECC
  if ($area_default2!=""){
    $area_default2=str_replace("''", ", ", $area_default2);
    $area_default2=str_replace("'", "", $area_default2);
  }
  $id_gerente=traer_fila_row(query_db("SELECT gerente FROM $co1 WHERE id=$id_contrato_arr"));
  $sel_v_usuario_area="SELECT * FROM $v_contra3 WHERE id_usuario=$id_gerente[0] AND id_area NOT IN($area_default2)";
  ...
  if ($area_default!=""){
    ...
    $area_default=str_replace("'", "", $area_default);
    $area_default=$area_default.", ".$area_default2;
  }
  //echo $id_contrato_arr."<BR>";
...
?>
  ...
  <table width="100%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
        <tr>
          ...
          <td width="50%" align="center" class="fondo_3"><div align="center">Area</div></td>
          <td width="30%" align="center" class="fondo_3"><div align="center">Usuario que Agreg&oacute;</div></td>
      ...
      <?
        ...
        $sel_v_usuario_area="SELECT * FROM $v_contra3 WHERE id_usuario=$id_gerente[0] AND id_area NOT IN($area_default2) ORDER BY NOMBRE ASC";
        $usuario_area=query_db($sel_v_usuario_area);
        while($area=traer_fila_row($usuario_area)){//INICIO WHILE USUARIO_AREA
      ?>
        <tr class="filas_resultados">
...
<input name="id_item" type="hidden" value="<?=$sel_id_item[0]?>" />
