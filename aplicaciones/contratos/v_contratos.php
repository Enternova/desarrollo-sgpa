...
$es_profesional_aseguramiento = traer_fila_row(query_db("select count(*) from tseg12_relacion_usuario_rol where id_usuario in (".$_SESSION["usuarios_con_reemplazo"].") and id_rol_general in (24)"));
/** PARA EL DES-017-17*****/	
$tipo_contrato='';
/** PARA EL DES-017-17*****/
?>
       ...
       <tr <?=$display?>>
		 ...
		 ?></td>
           ...
           <tr>
             ...
             <!-- pra el desarrollo de desempeno -->
             <tr>
             <td align="right"><strong>Tipo de servicio:</strong></td>
             <td colspan="2" align="left"><? if($edita==1){?>
              	<select name="tipo_servicio" id="tipo_servicio">
	             	<option value="0">Seleccione</option>
	             	<option value="1" <?if($sql_con[60]==1) echo "selected='selected'";?>>Contratos Obra, perforaci&oacute;n y s&iacute;simica</option>             	
	             	<option value="2" <?if($sql_con[60]==2) echo "selected='selected'";?>>Asesor&iacute;as,  Consultor&iacute;as e ingenier&iacute;a</option>
	             	<option value="3" <?if($sql_con[60]==3) echo "selected='selected'";?>>Otros</option>
	             </select>
               <? }else{?>
               <input type="hidden" name="tipo_servicio" id="tipo_servicio"  value="<?=$sql_con[60];?>" />
               <?
				if($sql_con[60]==1){
					echo "Contratos Obra, Perforaci&oacute;n y S&iacute;simica";
				}elseif($sql_con[60]==2){
					echo "Asesor&iacute;as,  Consultor&iacute;as e Ingenier&iacute;a";
				}elseif($sql_con[60]==3){
					echo "Otros";
				}else{
					echo "";
             ...
             <!-- fin pra el desarrollo de desempeno -->
             <?
             if($edita_2==1){
			 ?>
           <tr>
             <td align="right"><strong>Gerente de Contrato:</strong><br />confirmado por aseguramiento administrativo </td>
             <td colspan="2" align="left"><input name="gerente_confirma_asegu" type="text" id="gerente_confirma_asegu" size="5" value="<?=$nombre_generete_asegurameinto;?>"  onkeyup="selecciona_lista()" style="z-index: 1;"/></td>
           </tr>
	           ...
	           <tr >
	             ...
	             <td colspan="2" align="left">
	             	<select name="gerente_antiguo">
	             		<option value="0">Seleccione</option>
		             	<?
		             		/***** SI NO TIENE GERENTES ANTERIORES INGRESA EL ACUAL UNA VEZ *****/
		             		$query="SELECT COUNT(*) FROM t7_contratos_modica_gerente AS t, t1_us_usuarios as u WHERE u.us_id=t.id_gerente AND id_contrato=$id_contrato_arr";
		             		$total_gerente=traer_fila_row(query_db($query));
							if($total_gerente[0]==0){
								$query="INSERT INTO t7_contratos_modica_gerente(id_gerente, id_modifica, id_contrato, fecha_modifica) VALUES(".$sql_con[9].",".$_SESSION["id_us_session"].",".$id_contrato_arr.",'".date('Y-m-d')."')";
               ...
               <table width="99%" border="0" cellpadding="2" cellspacing="2">
                 <tr>
                   <td align="left"><? if($edita == 1){?><select name="porcen_garantia" id="porcen_garantia" onchange="">
                     ...
                     <option value="3" <? if($sql_con[54]==3){echo "selected='selected'";}?>>3%</option>
                     <option value="5" <? if($sql_con[54]==5){echo "selected='selected'";}?>>5%</option>
                     </select><? }else{ if($sql_con[54]==1) echo "1%"; if($sql_con[54]==5)echo "5%"; if($sql_con[54]==3)echo "3%";} ?></td>
	...
	?>
      ...
      <tr <?=$display?>>
        ...
        if($alert_validacion_valor_contra != ""){
			$funci = "muestra_alerta_error_solo_texto('', 'Error', '* ".$alert_validacion_valor_contra."', 20, 10, 18)";
			}
