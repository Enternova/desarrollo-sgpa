<? include("../../librerias/lib/@session.php"); 
  ...
  $area_item=traer_fila_row(query_db("SELECT DISTINCT p.t1_area_id, a.nombre FROM t2_item_pecc AS p, t1_area AS a WHERE a.t1_area_id=p.t1_area_id AND p.estado BETWEEN 20 AND 32 AND (p.id_item_peec_aplica=$sel_id_item[0] OR p.id_item=$sel_id_item[0])"));
  $area_default=$area_default."'".$area_item[0]."'";
  $id_gerente=traer_fila_row(query_db("SELECT gerente FROM $co1 WHERE id=$id_contrato_arr"));
   //echo $sel_v_usuario_area="SELECT * FROM $v_contra3 WHERE id_usuario=$id_gerente[0] AND id_area NOT IN($area_item[0])";
