...
$selec_t_pro = query_db("select area, Count(*), t1_area_id from indicador_reporte_niveles_aprobacion_adjudicacion_comite where id_item>0 $comple_sql group by area, t1_area_id");
...
$selec_t_pro = query_db("select area, t1_area_id from indicador_reporte_niveles_aprobacion_adjudicacion_comite where id_item>0  and t1_tipo_proceso_id <> 8 $comple_sql group by area, t1_area_id");

	
while($sel_pro = traer_fila_db($selec_t_pro)){

$selec_valor_ad = query_db("select id_item from indicador_reporte_niveles_aprobacion_adjudicacion_comite where id_item>0 $comple_sql and t1_area_id = ".$sel_pro[1]." and t1_tipo_proceso_id <> 8  group by id_item");
