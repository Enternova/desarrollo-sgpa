...
<body>
	...
	$tr3='<tr><td style=" background-color: #229BFF;">ROL</td><td style=" background-color: #229BFF; width: 150px !important;">USUARIO</td>';
	$arma_tabla='<table  border="1" class="tabla_lista_resultados"><tr><td align="center" bgcolor="#229BFF" class="fondo_3" style="width: 150px !important;"></td><td align="center" bgcolor="#229BFF" class="fondo_3"></td>';
	...
	while($sel_usuario2 = traer_fila_db($sel_usuario)){
		$tr4='<tr><td>'.$sel_usuario2[2].'</td>'.'<td>'.$sel_usuario2[1].'</td>';
		$sel_modulos = query_db("select nombre_modulo, id_modulo from v_roles_des099 group by orden_modulo, nombre_modulo, id_modulo order by orden_modulo");
		while($sel_mod = traer_fila_db($sel_modulos)){
			$sel_modulos2 = query_db("select nombre_permiso, id_permisos_modulo, nombre_modulo, id_modulo from v_roles_des099 where id_modulo=".$sel_mod[1]." group by orden_modulo, orden_premiso, nombre_permiso, id_permisos_modulo, nombre_modulo,id_modulo order by orden_modulo, orden_premiso");
			while($sel_mod2 = traer_fila_db($sel_modulos2 )){
				$cont_row_span2=traer_fila_row(query_db("select count(*) from v_roles_des099 where id_modulo=".$sel_mod[1]." and id_permisos_modulo=".$sel_mod2[1]." and us_id=".$sel_usuario2[0]." and id_rol=".$sel_usuario2[3]));
