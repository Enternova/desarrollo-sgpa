...
<tr>
  ...
  $titulo2="";
    		 ...
    		 $comple_sql_ots =" and t1.id_item_ots_aplica=".$sel_temp[2];	 
			 
	  if($sel_temp[1]=="inicial"){	 
	  $comple_sql =" and id_item_ots_aplica=0";
	  $comple_sql_ots =" and t1.id_item_ots_aplica=0"; 
  ...
  <tr class="" id="fila_2-<?=$consecutivo?>" >
    <td colspan="10" align="center" class="estilo_reporte_fondo_verde"><strong ><?=$titulo?> </strong></td>
    </tr>
    <tr class="fondo_3">
	...
	}else{ if($sel_temp[1]<>"inicial2"){?><td align="center"><?=$s_valores[4]?></td><td align="center"><?=$s_valores[12]?></td><? }}//si no es el primero debe imprimir la columna de los contratos sin rowspan
	...
	$valor_eqi_t=$valor_equivalente+$valor_eqi_t;
      ...
      <td width="1%" align="center"><?=number_format($s_valores[7],0)?></td>
      <td width="1%" align="center"><?=number_format($s_valores[8],0)?></td>
      <td width="1%" align="center"><?=number_format($valor_equivalente,2)?></td>
      <td width="1%" align="center"><?=number_format($trm,0)?></td>
    </tr>
...
$trm=trm_presupuestal($s_valores[0]);
	...
	if($moneda == "COP"){
      ...
      <td align="center"><?=number_format($valor_equivalente_solicitud,2)?></td>
...
<tr class="estilo_reporte_fondo_verde" id="fila_2-<?=$consecutivo?>" >
    <td colspan="10" align="center" class="estilo_reporte_fondo_verde">EJECUCION POR ORDENES DE TRABAJO CONTRATO <?=$sel_ots_cuadr[0]?></td>
    </tr>
...
<? 
	...
	if($moneda == "COP"){
      ...
      <td align="center" ><?=number_format($s_valores[1],0)?></td>
      <td align="center" ><?=number_format($s_valores[2],0)?></td>
      <td align="center" ><?=number_format($valor_equivalente_ots,2)?></td>
      <td align="center" ><?=number_format($trm,0)?></td>
    </tr>
...
<tr>
      <td colspan="10" height="1" align="center" class="estilo_reporte_fondo_verde">SALDO DISPONIBLE PARA CREAR ORDENES DE TRABAJO A LA SOLICITUD <?=$sel_temp[0]?></td>
      </tr>
...
<tr class="fondo_3">
 ...
 <?
	...
	if($es_el_primero == 0){//es para imprimir o no la primera columna que tiene el rowspan
		$es_el_primero =1;
    ?>
      <td colspan="2"  rowspan="<?=$convina_rowspan_columna_1?>" align="center">SALDO DE LA SOLICITUD <?=$sel_temp[0]?></td>
      <?
      if($sel_temp[1]=="inicial2"){//si es iniicial
	  ?>
	...
	}else{ if($sel_temp[1]<>"inicial2"){?><td align="center"><?=$s_valores[4]?></td><td><?=$s_valores[12]?></td><? }}//si no es el primero debe imprimir la columna de los contratos sin rowspan
...
$solo_contratos = str_replace("<font color=blue>","",$s_valores[4]);
...
$solo_contratos_explode = explode("C",$solo_contratos);
$cuatos_contratos =  count($solo_contratos_explode);
...
for($i = 0; $i <= $cuatos_contratos; $i++){
	if($solo_contratos_explode[$i] != ""){
$query="select  sum(t1.usd), sum(t1.cop) from t2_reporte_marco_temporal as t1, t2_item_pecc as t2 where t1.id_item = t2.id_item and t2.t1_tipo_proceso_id = 8 and t1.id_us= '".$_SESSION["id_us_session"]."' and t1.tipo = 'ots' and t1.ano =  '".$s_valores[5]."' and t1.id_campo = '".$s_valores[9]."'".$comple_sql_ots." AND REPLACE(t1.contratos, ' ', '') = 'C".$solo_contratos_explode[$i]."'";
      $selecciona_valores_ots = traer_fila_row(query_db($query));
	$valor_ot_usd=$valor_ot_usd + $selecciona_valores_ots[0];
	$valor_ot_cop=$valor_ot_cop + $selecciona_valores_ots[1];
	}
}
	$saldo_val_us = ($s_valores[7]-$valor_ot_usd);
	$saldo_val_cop = ($s_valores[8]-$valor_ot_cop);
	
	//$valor_equivalente = $valor_equivalente_solicitud - $valor_equivalente_ots;
	if($moneda == "USD"){
     $valor_equivalente = $saldo_val_us + ($saldo_val_cop/$trm);
	}
	if($moneda == "COP"){
     $valor_equivalente = $saldo_val_cop + ($saldo_val_us*$trm);
	}
	if($valor_equivalente<=0){
		$valor_equivalente=0;
		}
...
<tr>
  <td colspan="2" align="right"><strong style="cursor:pointer" onClick="abrir_ventana('../aplicaciones/reportes/detalle_reporte_saldos_excel.php?id_solicitud=<?=$_GET["id_solicitud"]?>&eq_moneda=<?=$eq_moneda?>&id_contrato=<?=$_GET["id_contrato"]?>')" >Exportar reporte a Excel <img src="../imagenes/mime/xlsx.gif"  /></strong></td>
</tr>
<tr>
