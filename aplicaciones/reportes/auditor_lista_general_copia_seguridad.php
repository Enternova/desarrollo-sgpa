...
<table width="100%" border="0" class="tabla_lista_resultados">
  ...
  $cont = 0;
   ...
   $sql_consulta = "select * from (select num1, num2, num3, tipo_log, nombre_administrador, fecha, hora_seg,tipo_log_sub, id_log, ROW_NUMBER()Over(order by fecha desc, hora_seg desc) As RowNum, id_tipo_log, id_modulo, consecutivo, ano, apellido,id_tipo_log_sub_ventana,de_historico from $vista_aplica where (id_log > 1 $sql_comple) $sql_comple_modulo group by num1, num2, num3, tipo_log, nombre_administrador, fecha, hora_seg,tipo_log_sub,id_log,id_tipo_log, id_modulo, consecutivo, ano, apellido,id_tipo_log_sub_ventana,de_historico ) as resultado_paginado WHERE RowNum BETWEEN $regis_inicial AND $regis_final ORDER BY id_log DESC";
		...
		?>
		<tr><td colspan="3"><?=$sql_consulta?></td></tr>
...
if($_SESSION["id_us_session"]==32){
  ...
  while($s_log = traer_fila_db($sel_logs)){
	  ...
	  $sel_cuantos_registros = traer_fila_Row(query_db("select count(*) from tseg9_log_detalle where id_log = ".$s_log[8]));
		...
		if($sel_cuantos_registros[0] > 1 and $_GET["modulo"] == 2){
				$link = '<a class="fancybox fancybox.iframe" href="auditor_detalle_general.php?id_log='.$s_log[0].'&vista_aplica=vista_log_3_usuarios_r">'.$s_log[3].' '.$tipo_sub[1].' </a>';
			}elseif($sel_cuantos_registros[0] > 1 or ($s_log[10] >= 18 and $s_log[10] <= 21) or $s_log[15] == 14 or $s_log[10] == 41){
...
<?
  ...
  <tr>
    ...
    <td width="19%"><?
          ...
          $cuantos_registros_sql = query_db("select count(*) from vista_log_3_usuarios where (id_log > 1 $sql_comple) $sql_comple_modulo group by id_log ORDER BY id_log DESC");
		  while($s = traer_fila_db($cuantos_registros_sql)){
			  $cuantos_registros = 1 +$cuantos_registros;
			  }
...
<table width="100%" border="0" class="tabla_lista_resultados">
  ...
  <?
	$query="select * from (select id_log, id_tipo_log, tipo_log, usuario_modificado, nombre_administrador, fecha, hora_seg, ROW_NUMBER()Over(order by id_log DESC, fecha desc, hora_seg desc) As RowNum from vista_log_3_usuarios where (id_log > 1 $sql_comple) $sql_comple_modulo) as resultado_paginado WHERE RowNum BETWEEN $regis_inicial AND $regis_final ORDER BY RowNum ASC";
		$cont = 0;
	...
	while($s_log = traer_fila_db($sel_logs)){
	 ...
	 $sel_cuantos_registros = traer_fila_Row(query_db("select count(*) from tseg9_log_detalle where id_log = ".$s_log[0]));
	  	  	  
		if($sel_cuantos_registros[0] > 1 ){
				$link = '<a class="fancybox fancybox.iframe" href="auditor_detalle_general.php?id_log='.$s_log[0].'&vista_aplica=vista_log_3_usuarios_r">'.$s_log[3].' '.$tipo_sub[1].' </a>';
			}else{
				$link = $s_log[2];
				}
