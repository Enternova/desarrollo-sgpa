...
$cont = 0;
 $query_reporte = "select * from vista_reporte_saldos_marco_1 where id_item > 0 ".$comple_sql.$sq_comple." and t1_tipo_documento_id=2  order by id_item desc";
  ...
  */
  ...
  ?>
      <tr class="fondo_3">
        ...
        <td colspan="5" align="center">Aprobaciones <?=contratos_relacionados_solicitud_para_campos($id_item_pecc)?></td>
        <td align="center">&nbsp;</td>
        <td align="center" class="fondo_4">Ejecuciones</td>
      ...
      <tr class="fondo_3">
        ...
        <td align="center" class="fondo_4">Equivalente Ejecuciones
        <?=$moneda?>
        $</td>
        <td align="center">Equivalente Saldo 
          <?=$moneda?>
          $</td>
   </tr>
...
/*nuevo query*/
    ...
    ?>
	  ...
	  ?>
		...
		echo '<td align="center" class="'.$classe2.'">'.$contratos.'</td>';
		?>
...
//echo $solo_contratos." - <br>";
$solo_contratos_explo = explode("C", $solo_contratos);
for($i=0; $i<=count($solo_contratos_explo); $i++){//for para recorer todos los contratos de la ampliacion o solicitud inicial o reclasificacion
if($solo_contratos_explo[$i]!=""){
$num_contrato_compara = $solo_contratos_explo[$i];
$complemtento_filtro_contrato = " and REPLACE(contratos, ' ', '') like '%".$num_contrato_compara."%'";

//$sql_ots = "select  ano, campo, sum(saldo_eq_usd), sum(saldo_eq_cop) from ".$tabla_aplica." where id_us= '".$_SESSION["id_us_session"]."' and tipo in ('ots') $comple_sql_ots $comple_sql_ots_sol $complemtento_filtro_contrato group by ano, campo order by ano,campo asc";

$sql_ots = "select  t1.ano, t1.campo, sum(t1.saldo_eq_usd), sum(t1.saldo_eq_cop) from ".$tabla_aplica." as t1, t2_item_pecc as t2 where t1.id_item = t2.id_item and t2.t1_tipo_proceso_id = 8 and t1.id_us= '".$_SESSION["id_us_session"]."' and t1.tipo in ('ots') $comple_sql_ots $comple_sql_ots_sol $complemtento_filtro_contrato group by t1.ano, t1.campo order by t1.ano,t1.campo asc";


//echo $num_contrato_compara."<br><br>";
		   $selecciona_valores_ots = query_db($sql_ots);
			   
				while($s_valores_ots = traer_fila_db($selecciona_valores_ots)){
	$trm_ots=trm_presupuestal($s_valores_ots[0]);       	 
if($moneda == "USD"){
//		$eq_usd_ots = $s_valores_ots[2] + ($s_valores_ots[3]/$trm_ots);	
		$valor_equivalente_ots = $valor_equivalente_ots + $s_valores_ots[2];//--
}
if($moneda == "COP"){
	//	$eq_usd_ots = $s_valores_ots[3] + ($s_valores_ots[2]*$trm_ots);	
		$valor_equivalente_ots = $valor_equivalente_ots + $s_valores_ots[3];//--
}
		 // $valor_usd_ots=$valor_usd_ots + $s_valores_ots[2];
		  //$valor_cop_ots= $valor_cop_ots+$s_valores_ots[3];
		 // $valor_equivalente_ots=$valor_equivalente_ots + $eq_usd_ots;
...
/*suma ejecuciones*/
    ...
    ?>
        <td id="fila_9-<?=$consecutivo?>" rowspan="<?=$convina_rowspan_columna_1?>" align="center" class="<?=$classe2?>"><?=number_format($valor_equivalente_ots,0,'','')?></td>
		<td id="fila_9-<?=$consecutivo?>" rowspan="<?=$convina_rowspan_columna_1?>" align="center" class="<?=$classe2?>"><?=number_format($valor_equivalente_saldo,0,'','')?></td>
	...
	?>
      <tr >
        ...
        <td align="center" class="fondo_3"><?=number_format($valor_equ_total_liz,0,'','')?></td>
        <td align="center"></td>
		<td align="center" class="fondo_4">
        <?=number_format($valor_equivalente_ots_total,0,'','')?>
        </td>        
        
        <td align="center" class="fondo_3"><?=number_format($valor_equ_total_liz-$valor_equivalente_ots_total,0,'','')?></td>
