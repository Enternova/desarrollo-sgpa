...
$trae_id_insrte = " select SCOPE_IDENTITY() AS [SCOPE_IDENTITY]"; // para optener elid del insert into SQL SERVER
	...
	if($_GET["tipo_seleccion"]){
		if($_GET["tipo_seleccion"]==3){
			$nombre_tipo_documento="Orden de Pedido";
		}else{
			$query="select nombre where t1_tipo_documento_id=".$_GET["tipo_seleccion"];
			$nombre_tipo_documento=traer_fila_row(query_db($querry));
  ...
  <tr>
    ...
    <?
          if ($edicion_datos_generales == "SI"){
			  if($sel_item[6] <> 6 and $sel_item[6] <> 15){
		  ?>
...
?>
    <br />
      ...
      <tr>
        ...
        <select name="tipo_documento" id="tipo_documento" onchange="valida_tipo_doc(<?=$id_item_pecc?>, this.value)">
          <option value="">Tipo del Documento</option>
           	<?
	        	if($_GET["tipo_seleccion"]){
	        		if($_GET["tipo_seleccion"]==3){
	        			echo '<option value="3" selected="selected">Orden de Pedido</option>';
	        		}else{
	        			echo '<option value="3">Orden de Pedido</option>';
	        		}
		...
		/*	  
          if($sel_item[6]==15 and $la_solicitud_que_modifica!="adjudicacion" and $sel_item[21]==0){//si es modificacion y permiso
		  
		  ?><input type="hidden" name="sele_proveedor" id="sele_proveedor" value="15957" /> <strong class="letra-descuentos">Por ser modificaci&oacute;n de una solicitud de permiso no requiere ingresar proveedores.</strong><?
		  
		  
		  }else{*/
		  ?>
          <select name="sele_proveedor" id="sele_proveedor" onchange="carga_otros_proveedores(this.value)">
			...
			?>
            <option value="<?=$sel_cont[0]?>"><?=$sel_cont[1]?></option>
            <?
				}
				...
				while($sel_cont = traer_fila_db($sele_proveedores)){
					?>
            <option value="<?=$sel_cont[1]?>"><?=$sel_cont[0]?></option>
            <?
				}
      ...
      <tr>
        <td colspan="4" align="right"><input name="button2" type="button" class="boton_grabar" id="button2" value="Agregar Linea de Adjudicaci&oacute;n" onclick="muestra_alerta_general_desde_select('graba_presupuesto_nuevo_edicion_adjudicacion()', 'Advertencia', 'Aseg&uacute;rese que el proveedor <campo> Adjudicado, fue aprobado previamente para participar en el proceso', 'sele_proveedor')" /></td>
        </tr>
    </table>
	...
	<tr>
	 ...
	 <!--- PARA EL INC-008 2017 -->
	  ...
	  <td width="50%" colspan="2" valign="top"><table width="50%" border="0" class="tabla_lista_resultados">
	    <tr>
	      <td align="center"  class="fondo_3" width="40%">Proveedor</td>
	      <td align="center"  class="fondo_3" width="30%">Tipo de Compra</td>
	      <td align="center"  class="fondo_3">Total USD</td>
	      <td align="center"  class="fondo_3">Total COP</td>
	      </tr>
	    <?php 
