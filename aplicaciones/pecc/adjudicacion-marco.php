...
<table width="100%"  border="0" cellpadding="2" cellspacing="2" align="center" <? if ($desde_comite == "SI") {echo 'bgcolor="#FFFFFF" width="95%"';} else {echo 'width="100%"';}?>>
  ...
  <tr>
	...
	//require('muestra_tipo_documento.php');
		//echo ayuda_alerta("Tipo de Esta Adjudicaci&oacute;n: Compra / Contrataci&oacute;n Marco");
	/***** PARA EL INC-008 *********/
  	?>
    <?
      ...
      <tr>
		...
		?>
              <select name="tipo_documento" id="tipo_documento" onchange="valida_tipo_doc(<?=$id_item_pecc?>, this.value)">
                ...
                <?
	        		...
	        		echo listas_sin_seleccione($g17, " estado = 1 ",$_GET["tipo_documento"] ,'nombre', 1);
	        	}
	       		?>
              </select>
              <?
		...
		?></td>
          ...
          <tr>
            ...
            <td align="left"><select name="sele_proveedor" id="sele_proveedor" onchange="carga_otros_proveedores_adjudicacion(this.value)">
...
<?
  ...
  $clase="";
		...
		?><table width="100%" border="0" align="center"  class="tabla_lista_resultados">
          <tr>
            <td colspan="6" align="center"  class="fondo_3">Valor de la Adjudicaci&oacute;n del Contrato Marco</td>
          </tr>
  ...
  $sele_presupuesto = query_db("select * from $vpeec25 where t2_item_pecc_id ='".$id_item_pecc."' order by t1_campo_id, ano");
	...
	$total_equivale_usd = 0 ;
		$cont = 0;
  $clase="";
   $delete_temporal = query_db("delete from  t2_reporte_temporal_adjudicacion  where id_us=".$_SESSION["id_us_session"]." and  id_item=".$id_item_pecc."");
        	while($sel_presu = traer_fila_db($sele_presupuesto)){
				$valor_total_usd = $valor_total_usd + $sel_presu[3];
				$valor_total_cop = $valor_total_cop + $sel_presu[4];
		...
		?>
		  ...
		  ?>
			...
			$linea_ad_proveedor ="";
			while($sel_apl = traer_fila_db($sel_contr)){
					$linea_ad_proveedor.="* ".$sel_apl[0]." <strong>".tipo_bien_servicio_sin_contrato(str_replace(".","",$sel_apl[1]))."</strong><br />";
					echo "* ".$sel_apl[0]." <strong>".tipo_bien_servicio_sin_contrato(str_replace(".","",$sel_apl[1]))."</strong><br />";
			}
		  ?></td>
		  ...
		  $insert_temporal = query_db("insert into t2_reporte_temporal_adjudicacion (id_us, id_item, proveedor, ano, area, usd, cop) values ('".$_SESSION["id_us_session"]."', '".$id_item_pecc."', '".$linea_ad_proveedor."', '".$sel_presu[1]."', '".$sel_presu[2]."', '".$sel_presu[3]."', '".$sel_presu[4]."')");
			}
			  ...
			  $sel_ontrato = traer_fila_row(query_db("select creacion_sistema, consecutivo, apellido, id from $co1 where id = '".$sel_presu[0]."'"));
				...
				if($sel_ontrato[0] != ""){
					...
					echo numero_item_pecc_contrato($numero_contrato1,$numero_contrato2,$numero_contrato3, $numero_contrato4, $sel_ontrato[3])." Contrato Marco";
					 $linea_para_todos.="* ".$sel_presu[1]." ".numero_item_pecc_contrato($numero_contrato1,$numero_contrato2,$numero_contrato3, $numero_contrato4, $sel_ontrato[3])."<br />";
				}else{
					 $linea_para_todos.="* ".$sel_presu[1]." ";
					echo "Sin Crear";
					}
		?></td>
          <td align="center"><?=$sel_presu[2]?></td>
...
//			if($_SESSION["id_us_session"] == "32"){
      ...
      <?
      echo ayuda_alerta("Balance de la Adjudicaci&oacute;n")
	  ?>

      <table width="100%" border="0">
        <tr>
          <td valign="top"><table width="100%" border="0" class="tabla_lista_resultados">
	    <tr>
	      <td align="center"  class="fondo_3" width="60%">Proveedor</td>
	      <td align="center"  class="fondo_3" width="10%">A&#328;o</td>
	      <td width="14%" align="center"  class="fondo_3">Total USD</td>
	      <td width="16%" align="center"  class="fondo_3">Total COP</td>
	      </tr>
	    <?php 
				
				
				$group_presupuesto_ano = query_db("select CAST(proveedor as TEXT), ano, SUM(usd), SUM(cop) from t2_reporte_temporal_adjudicacion where id_us=".$_SESSION["id_us_session"]." and  id_item = $id_item_pecc  group by proveedor, ano");
                $cont = 0;
                ...
                if($cont == 0){
                    ...
                    ?>
	    <tr class="<?=$clase?>">
	      <td><? if($rowGPA[0]=="" or $rowGPA[0]==" " or $rowGPA[0]=="  ") echo $linea_para_todos; else echo $rowGPA[0];?></td>
	      <td align="center"><?= $rowGPA[1]?></td>
	      <td align="center"><?= number_format($rowGPA[2])?></td>
	      <td align="center"><?= number_format($rowGPA[3])?></td>
	      </tr>
	    <?php }?>
	    </table></td>
          <td rowspan="2" valign="top"><table width="100%" border="0" class="tabla_lista_resultados">
	    <tr>
	    ...
	    <?php 
               // $group_presupuesto_area = query_db("select razon_social,nombre,sum(valor_usd) as valor_usd,sum(valor_cop) as valor_cop from $vpeec18 where t2_item_pecc_id = $id_item_pecc   group by razon_social,nombre");
				$group_presupuesto_area = query_db("select CAST(proveedor as TEXT), area, SUM(usd), SUM(cop) from t2_reporte_temporal_adjudicacion where id_us=".$_SESSION["id_us_session"]." and  id_item = $id_item_pecc  group by proveedor, area");
        ...
        <tr>
          <td valign="top"><table width="100%" border="0" class="tabla_lista_resultados">
            <tr>
              <td align="center"  class="fondo_3" width="70%">Proveedor</td>
              <td width="15%" align="center"  class="fondo_3">Total USD</td>
              <td width="15%" align="center"  class="fondo_3">Total COP</td>
              </tr>
            <?php 
				
				
				$group_presupuesto_ano = query_db("select CAST(proveedor as TEXT), SUM(usd), SUM(cop) from t2_reporte_temporal_adjudicacion where id_us=".$_SESSION["id_us_session"]." and  id_item = $id_item_pecc  group by proveedor");
            ...
            <tr class="<?=$clase?>">
              <td><? if($rowGPA[0]=="" or $rowGPA[0]==" " or $rowGPA[0]=="  ") echo $linea_para_todos; else echo $rowGPA[0];?></td>
              <td align="center"><?= number_format($rowGPA[1])?></td>
              <td align="center"><?= number_format($rowGPA[2])?></td>
              </tr>
            <?php }?>
          </table></td>
