...
<table width="100%" border="0" cellpadding="2" cellspacing="2">
  ...
  <tr>
    <td width="77%" height="445" valign="top">
    <?
    ...
    <table width="100%" border="0" align="center"  class="tabla_lista_resultados">
      ...
      <tr>
        ...
        <td width="49%" align="left"><select name="rol_encarga_permiso" id="rol_encarga_permiso">
          <?=listas("tseg2_permisos", " estado = 1 and id_modulo = 1 and id_tipo_permiso = 2 and id_premiso not in (8,15, 35)",0 ,'nombre', 3);?>
...
<?
    ...
    <table width="100%" border="0" align="center"  class="tabla_lista_resultados">
        ...
        <tr>
          ...
          <td width="12%" rowspan="2" align="center" class="fondo_3">Usuarios Encargados</td>
          <?
       ...
       $sel_propuestos_real = query_db("select id_rol, rol,orden from $vpeec15 where id_item_pecc = ".$id_item_pecc." and tipo_adj_permiso = 2 and id_rol not in (10,11) group by id_rol, rol,orden order by orden");
		while($sel_p_real = traer_fila_db($sel_propuestos_real)){
			...
			$select_secuencia = traer_fila_row(query_db("select * from $pi14 where id_item_pecc = ".$id_item_pecc." and id_rol = ".$sel_p_real[0]." and tipo_adj_permiso = 2 and estado=1"));
        ...
        <tr>
          ...
          <td align="left"><?
          $sel_real_us = query_db("select * from $vpeec15 where id_item_pecc = ".$id_item_pecc." and id_rol = ".$sel_p_real[0]." and tipo_adj_permiso = 2 and estado = 1 order by nombre_administrador");
				while($sel_re_us = traer_fila_db($sel_real_us)){
					if($sel_re_us[4] <> ""){
					if($edita_permiso == "SI" and $edicion_datos_generales_permiso == "SI"){
					echo "*.".$sel_re_us[4]." <img src='../imagenes/botones/eliminada_temporal.gif' width='10' height='10' onclick='elimina_usuario_firma_adjudica(".$sel_re_us[7].")'/><br />";
					}else{
						echo "*.".$sel_re_us[4]." <br />";
						}
					}
       ...
       <?
          ...
          <?
		  	...
		  	if($sel_item[14] == 15 and $sel_p_real[0] == 15){
				echo "Pendiente";
			}else{
...
$selecciona_si_es_usuario_de_socios = traer_fila_row(query_db("select count(*) from $vpeec15 where id_item_pecc = ".$id_item_pecc." and id_rol = 11 and tipo_adj_permiso = 2 and estado = 1 and id_secuencia_solicitud = ".$select_secuencia[0]." and us_id = ".$_SESSION["id_us_session"].""));
        ...
        <tr class="filas_resultados">
          ...
          <td align="center"><?=$sel_p_real[1]?></td>
          <td align="left"><?
          $sel_real_us = query_db("select * from $vpeec15 where id_item_pecc = ".$id_item_pecc." and id_rol = 11 and tipo_adj_permiso = 2 and estado = 1 order by nombre_administrador");
				while($sel_re_us = traer_fila_db($sel_real_us)){
					if($sel_re_us[4] <> ""){
