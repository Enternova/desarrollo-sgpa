...
<? if($edicion_datos_generales_pecc == "SI"){ ?>
      ...
      <option value="0">Seleccione</option>
       ...
       <?
          $lineas_pecc = query_db("select * from t1_lineas_pecc as t1 where estado = 1");
	...
	if($sel_item[71] > 0){
	$sel_linea = traer_fila_row(query_db("select codigo, detalle from t1_lineas_pecc where id = '".$sel_item[71]."'"));
...
?><? }?></strong></td>
      ...
      <tr>
        ...
        <?
        	...
        	if(($sel_item[6] <> 8) and ($sel_item[6] <> 7) and $edicion_datos_generales == "SI"){
		...
		?>
          <select name="tipo_proceso" id="tipo_proceso"  <?=$funti ?>>
			...
			echo listas($g13, " estado = 1 and t1_tipo_proceso_id  not in (".$quita_pone_adjudica_directo." 7, 8, 5, 15)",$sel_item[6] ,'nombre', 1);	
...
?>
      ...
      <tr>
        ...
        <td colspan="3">
        <?
        	if(($sel_item[6] <> 8) and $edicion_datos_generales == "SI"){
		?>
        <textarea name="alcance" id="alcance" cols="25" rows="3"><?=$sel_item_obs[2]?></textarea>
        <?
        }else{
				echo $sel_item_obs[2];
...
?>
		...
		echo $sel_item_obs[4];?><input type="hidden" name="justificacion" id="justificacion" value="<?=$sel_item_obs[4]?>"/><?
...
?>
          <textarea name="equipo_negociador" id="equipo_negociador" cols="25" rows="4"><?=$sel_item_obs[14]?></textarea>          <?
			...
			echo $sel_item_obs[14];
			}
		?></td>
      </tr>
	  <?
	  }else{//cuando se active eliminar el else
	  ?><input type="hidden" name="equipo_negociador" id="equipo_negociador" value="No aplicaNo aplicaNo aplicaNo aplicaNo aplicaNo aplicaNo aplica" /><?
		 ...
		 ?> 
          ...
          <tr>
           ...
           <?
			 ...
			 ?>
             <input type="hidden" name="campos4" id="campos4" value="<?=$p_optimizar?>" />
			 ...
			 <input type="hidden" name="campos7" id="campos7" value="<?=$p_sostenibilidad?>" />
			 <?
			 }
		  ?>
        </table></td>
	...
	$falta_algun_afe_ceco = 0;
      ...
      <tr>
        ...
        <table width="968" border="0" class="tabla_lista_resultados">
          ...
          <tr>
            <td align="center" class="fondo_3">Observacion</td>
...
if(($sel_item[6] == 7 or $sel_item[6] == 8) or $edicion_datos_generales == "NO"){
?>
