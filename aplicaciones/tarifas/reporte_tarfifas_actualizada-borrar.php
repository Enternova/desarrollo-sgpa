...
$busca_datos_tarifa_actual = "select t6_tarifas_lista_id, categoria, grupo, detalle, unidad_medida, moneda, valor, fecha_inicio_vigencia, tipo_creacion_modifica,us_aprobacion_actual,creada_luego_firme from v_tarifas_lista_estados where t6_tarifas_lista_id = $id_tarifa";
...
if(trim($busca_soporte_creacion[3])!=""){
	 $descraga_documento = "<a href='javascript:void(0)' onclick='javascript:window.parent.location.href=\"../aplicaciones/tarifas/proveedor/descarga_anexo_m.php?id_documen=".$busca_soporte_creacion[0]."\"'>".$busca_soporte_creacion[3]."</a>";
	 }
...
<body>
      ...
      <table width="99%" border="0" cellspacing="2" cellpadding="2">
        <tr>
          ...
          <td width="13%" >
          <? if($ruta_apro==1){ ?>
          <input type="button" name="button5" class="boton_volver"  id="button5" value="Volver al historico" onclick="ajax_carga('../aplicaciones/tarifas/h_aprobaciones.php?id_contrato=<?=$id_contrato;?>','carga_acciones_permitidas')" />
		<? } elseif($ruta_apro==2){ ?>
...
<? } ?>
        ...
        <tr>
          ...
          <td colspan="2" ><span class="titulos_resaltado_subtitulos_contenidostarifas"><?=$sql_con[6];?></span></td>
        </tr>
        <tr>
...
<?
	...
	$sql_busca_todas_aprobaciones=query_db($busca_todas_aprobaciones);
      ...
      <table width="99%" border="0" align="center" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
        ...
        <tr>
          <td colspan="2"><table width="99%" border="0" cellspacing="3" cellpadding="3">
            ...
            <tr>
              <td width="16%">&nbsp;</td>
              <td width="16%" class="columna_titulo_resultados">Usuario responsable</td>
              <td width="16%" class="columna_titulo_resultados">Fecha de aprobaci&oacute;n</td>
              <td width="9%" class="columna_titulo_resultados">Aprobaci&oacute;n</td>
              <td width="43%" class="columna_titulo_resultados">Comentarios</td>
            </tr>
     ...
     <?
			...
			?>             
            <tr>
              <td><div align="right"><strong>Especialista:</strong></div></td>
              <td><?=$resultado_apro[0];?></td>
              <td><?=$resultado_apro[1];?></td>
              <td><?=$resultado_apro[3];?></td>
              <td><?=$resultado_apro[2];?></td>
