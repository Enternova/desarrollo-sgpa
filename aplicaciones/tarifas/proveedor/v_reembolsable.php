<? include("../../../librerias/lib/@session.php"); 
    ...
    $busca_item = "select t6_tarifas_reembolables_datos_id, tarifas_contrato_id, fecha_creacion, estado, fecha_ini, fecha_fin, municipo, municipo ,porcentaje_administracion, consecutivo, editado
...
$consecutivo_reembol="R- ".$sql_ex[9]." - ".$fecha_cre[0]." ".$version;		
...
<table width="99%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="91%" class="titulos_secciones">SECCION:<span class="titulos_resaltado_procesos"> CONTRATO:
      <?=$sql_con[7];?>
    </span></span>&gt;&gt;  VISTA PREVIA DEL REEMBOLSABLE</td>
    <td width="9%" class="titulos_secciones">
<? 
if ($sql_ex[3]==1){ ?>
    <input type="button" name="button5" class="boton_volver" id="button5" value="Volver al historico" onclick="ajax_carga('../aplicaciones/tarifas/proveedor/h_reembolsable.php?id_contrato=<?=$id_contrato;?>','carga_acciones_permitidas')" />
<? } else { ?>
    <input type="button" name="button6" class="boton_volver" id="button6" value="Volver al tiquete de reembolsable" onclick="ajax_carga('../aplicaciones/tarifas/proveedor/e_reembolsable.php?id_contrato=<?=$id_contrato;?>&id_reembolsable_factura=<?=$id_reembolsable_factura_or;?>','carga_acciones_permitidas');" /></td>
<? } ?>
  </tr>
</table>
<br />
...
<table width="99%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
  ...
  <tr>
    <td><div align="left"><strong>Consecutivo del tiquete de servicio: <?=$consecutivo_reembol;?></strong></div></td>
  </tr>
  <tr>
   ...
   <tr>
    <td><div align="left"><strong>El contrato es tipo:</strong>  <? if($sql_ex[9]==1){ $text.='Marco | orden de trabajo:'.$sql_ex[10]; } else $text.='Normal'; ?>
	</div></td>
  </tr>  
...
<table width="99%" border="0" align="center" class="tabla_lista_resultados">
  ...
  <tr class="<?=$class;?>">
    ...
    <td><?=$l_ree[7];?></td>
    <td><?=$l_ree[8];?></td>
    <td><img src="../imagenes/mime/<?=extencion_archivos($l_ree[9]);?>.gif" width="16" height="16" onClick="window.parent.location.href='../enterproc/librerias/php/descarga_documentos_tarifas_reem.php?n1=<?=$l_ree[0];?>&n2=<?=$l_ree[9];?>'"/></td>
  </tr>
  ...
  <tr>
    <td class="titulos_secciones"><div align="right"> <span class="columna_subtitulo_resultados_mas_oscuro"> + ADMINISTRACION ( % <?=$sql_ex[8];?>) $ 
      <? $valor_admin = ($total*$sql_ex[8])/100;
	  	echo decimales_estilo($valor_admin,2);
...
<? if($sql_ex[3]==1){ ?>
            <td><div align="center">
              ...
              <? if($sql_ex[10]==0){ ?>
                ...
                <input name="button8" type="button" class="boton_email" id="button8" value="Editar reembolsable" onclick="edita_reem_en_firme(<?=$id_reembolsable_arr;?>)" />
              <? } ?>
              </p>
            </div></td>
<? } elseif($sql_ex[3]==2){ ?>           
            <td><div align="center"><input name="button" type="button" class="boton_email" id="button" value="Exportar reembolsable en borrador   con administraci&oacute;n" onclick="window.open('../enterproc/librerias/tcpdf/examples/exporta_reembolsable.php?id_contrato=<?=$id_contrato;?>&id_reembolsable_factura=<?=$id_reembolsable_factura_or;?>&adm=1')" />&nbsp;&nbsp;<input name="button3" type="button" class="boton_email" id="button3" value="Exportar reembolsable en borrador   sin administraci&oacute;n" onclick="window.open('../enterproc/librerias/tcpdf/examples/exporta_reembolsable.php?id_contrato=<?=$id_contrato;?>&id_reembolsable_factura=<?=$id_reembolsable_factura_or;?>&adm=2')" />
              <input name="button7" type="button" class="boton_email" id="button7" value="Exportar reembolsable en borrador  solo administraci&oacute;n" onclick="window.open('../enterproc/librerias/tcpdf/examples/exporta_reembolsable.php?id_contrato=<?=$id_contrato;?>&amp;id_reembolsable_factura=<?=$id_reembolsable_factura_or;?>&adm=3')" />
            </div></td>
<? } ?>            
          </tr>
        </table>
