...
<table width="99%" border="0" cellspacing="2" cellpadding="2">
  <tr>
    <td width="91%" class="titulos_secciones_tarifas">SECCION:<span class="titulos_resaltado_procesos_tarifas"> VARIACION DE TARIFAS&gt;&gt; CONTRATO:
      <?=$sql_con[7];?>
    </span></td>
...
$lista_existentes = $selec_lista[0];
  ...
  <tr>
    ...
    <td colspan="2" valign="top" class="fondo_6" ><div align="left">SELECCIONE UNA LISTA:
        <select name="lista_existentes" id="lista_existentes" class="select_ancho_automatico" onchange="ajax_carga('../aplicaciones/tarifas/h_variacion_tarifas.php?id_contrato=<?=$id_contrato;?>&lista_existentes=' + this.value,'carga_acciones_permitidas')">
...
if($estado_busca == 1 ){ $bus_tarifa_c.= " and t6_tarifas_estados_tarifas_id in (2, 3) and (creada_luego_firme = 2 or creada_luego_firme is null)";}//SI SELECCIONA VER LAS PENDIENTES
if($estado_busca == 2){ $bus_tarifa_c.= " and t6_tarifas_estados_tarifas_id in (1) and (creada_luego_firme = 2 or creada_luego_firme is null) and (fecha_fin_vigencia >= '$fecha' or fecha_fin_vigencia = '0000-00-00' ) ";}//SI SELECCIONA VER LAS APROBADAS
...
if($estado_busca == 6){ $bus_tarifa_c.= " and fecha_fin_vigencia < '$fecha' and  fecha_fin_vigencia <> '0000-00-00' ";}//SI SELECCIONA VER VENCIDAS
  ...
  $sql_cuenta2 = "select  count(*) from  $v_t_3 where tarifas_contrato_id = $id_contrato_arr  and t6_tarifas_listas_lista_id = $lista_existentes $bus_tarifa_c and tipo_creacion_modifica in (1,3) ";
...
<table width="98%" border="0" class="tabla_lista_resultados">
  ...
  <tr>
    ...
    <td>
    <input type="hidden" name="estado_busca" id="estado_busca" value="<?=$estado_busca?>" />
    </td>
    <td align="right">&nbsp;</td>
    <td colspan="2">&nbsp;</td>
  </tr>
  ...
  <tr>
    <td colspan="5" align="center"><input name="button" type="button" class="boton_buscar" id="button" value="      Buscar tarifas      "  onclick="javascript:document.principal.pagina.value=1;busqueda_paginador_nuevo_tarifas(1,'../aplicaciones/tarifas/h_variacion_tarifas.php','carga_acciones_permitidas',20)"/></td>
  </tr>
</table>
<p>&nbsp; </p>
<?=ayuda_alerta("Se Listan las tarifas contractuales y creadas como nuevas tarifas (Tarifas originales), para ver su variaci&oacute;n o modificaciones, debe exportar la lista a Excel")?><br />
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="2" cellpadding="2" class="tabla_paginador">
      <tr>
        ...
        <td width="12%">
          <select name="pagina" id="pagina" onchange="javascript:busqueda_paginador_nuevo_tarifas(this.value,'../aplicaciones/tarifas/h_variacion_tarifas.php','carga_acciones_permitidas',50)">
            <? 
 ...
 $nobre_categori_impri="";
		...
		$nombre_gupo_imprime="";
 	 	 ...
 	 	 $busca_detalle = "select * from (
								select * , ROW_NUMBER() OVER(ORDER BY categoria, grupo, tarifa_padre, fecha_creacion desc) AS rownum from $v_t_3 where tarifas_contrato_id = $id_contrato_arr and tipo_creacion_modifica in (1,3)   and t6_tarifas_listas_lista_id = $lista_existentes   $bus_tarifa_c
								) as sub
	  		...
	  		$nobre_categori_impri=$lista_detalle[2];
						...
						$busca_atributos=query_db("select * from $t13 where t6_tarifas_listas_lista_id = $lista_existentes and estado = 1");
									...
									<table border="0" cellspacing="2" cellpadding="2" width="100%" >                	
											...
											<th width="6%" class="fondo_3" align="center"><?=TITULO_CONSECUTIVO;?></th>
...
//echo "select t6_tarifas_lista_id, nombre_estado_tarifa from $v_t_3 where tarifa_padre = $lista_detalle[15] and t6_tarifas_lista_id <> $lista_detalle[15]  order by t6_tarifas_lista_id desc";
		$cuenta_tarifas_modificadas=traer_fila_row(query_db("select t6_tarifas_lista_id, nombre_estado_tarifa from $v_t_3 where tarifa_padre = $lista_detalle[15] and t6_tarifas_lista_id <> $lista_detalle[15] and tipo_creacion_modifica in (1,3)   order by t6_tarifas_lista_id desc"));
	...
	if( ($lista_detalle[19] != '0000-00-00') && ($lista_detalle[19] < $fecha AND $lista_detalle[13] != 7))
			{
...
<p>
		...
		<? if($id_ingreso[0]>=1){ ?>
           <table width="100%" border="0">
		     <tr>
		       <td align="center"><input name="button3" type="button" class="boton_buscar" id="button3" value="Exportar Excel y Ver Variaci&oacute;n"  onclick="javascript:exporta_tarifas_consulta_variacion()" /></td>
	         </tr>
</table>
...
<? } ?>
        <p>&nbsp;</p>
           ...
           <input type="hidden" name="tipo_reporte" id="tipo_reporte" value="h_variacion_tarifas" />
