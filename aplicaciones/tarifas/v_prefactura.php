...
<table width="99%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="93%" class="titulos_secciones">SECCION:<span class="titulos_resaltado_procesos"> CONTRATO:
      <?=$sql_con[7];?>
    </span></span>&gt;&gt;  VISTA PREVIA DE TIQUETE DE SERVICIO</td>
    <td width="7%" class="titulos_secciones">
<? 
if ($ruta_regreso==1){ ?>
    <input type="button" name="button5" class="boton_volver" id="button5" value="Volver al historico" onclick="ajax_carga('../aplicaciones/tarifas/h_prefactura_prove.php?id_contrato=<?=$id_contrato;?>','carga_acciones_permitidas')" />
<? } elseif ($ruta_regreso==2){ ?>
        <input type="button" name="button5" class="boton_volver" id="button5" value="Volver al historico" onclick="ajax_carga('../aplicaciones/tarifas/h_prefactura_prove_reporte.php?b_contrato=<?=$_GET["b_contrato"]?>&b_tiquete=<?=$_GET["b_tiquete"]?>&b_proyect=<?=$_GET["b_proyect"]?>&fecha_inicial=<?=$_GET["fecha_inicial"]?>&fecha_final=<?=$_GET["fecha_final"]?>&gerentes=<?=$_GET["gerentes"]?>&b_provee=<?=$_GET["b_provee"]?>&pagina=<?=$_GET["pagina"]?>','contenidos')" />
</td>
<? } ?>
  </tr>
  <tr>
...
<table class="tabla_principal">
  ...
  <tr>
    <td class="titulos_resaltado_procesos_tarifas">Consecutivo: '.$consecutivo_tiquete.'</td>
  </tr>
  <tr>
    <td class="titulos_resaltado_procesos_tarifas">Fecha del tiquete:'.fecha_for_hora($sql_busca_temporal_pre_fa[12]).'</td>
  </tr>
  <tr>
...
?>
  ...
  <table class="tabla_principal">
  <tr>
    <td class="titulos_resaltado_procesos_tarifas"><strong>Contrato:</strong>
      <?=$consecutivo_contrato;?>
    </td>
  </tr>
  <tr>
    <td class="celdas_largas"><span class="titulos_resaltado_procesos_tarifas"><strong>Rango de fechas del servicio prestado:</strong>
        <?=$fecha_inicial.' al '.$fecha_final;?>
    </span></td>
  </tr>
  <tr>
    <td class="celdas_largas"><strong>Objeto del Contrato:</strong><?=arr_caracteres_imprime($objeto_contarto);?></td>
  </tr>
   <tr>
...
$crea_campo_columna_sin="";
  $busca_detalle_modi_sin = "select valor, fecha_inicio_vigencia, fecha_fin_vigencia, consecutivo_tarifa from v_tarifas_lista_estados_fechas where tarifas_contrato_id = $id_contrato_arr and categoria = '$imprime_detalle[4]' 
		and grupo = '$imprime_detalle[5]' and t6_tarifas_estados_tarifas_id in (1,7) and fecha_arreglada_final between '$fecha_inicial' and '$fecha_final' and tarifa_padre = $imprime_detalle[11] and 
		...
		while($lis_modi_no_sell=traer_fila_row($sql_no_selecc_mo))
			$detall_m_no_sel.=' Consecutivo: '.$lis_modi_no_sell[3].'  Valor: '.decimales_estilo($lis_modi_no_sell[0],2).' fecha de finalizacion de vigencia '.$lis_modi_no_sell[2]." | ";
	...
	?>
     	...
     	<tr >
              ...
              <td class="celda_detalle_lista_completa"><?=decimales_estilo($imprime_detalle[7],2);?></td>
...
<p>&nbsp;</p>
        <table width="100%" border="0" cellspacing="2" cellpadding="2">
          <tr>
            ...
            <td width="30%"><div align="center"><input name="button" type="button" class="boton_email" id="button" value="Exportar tiquete de servicio en firme a PDF" onclick="window.open('generacion_tiquete_<?=$id_contrato;?>_<?=$id_prefactura;?>.html')" /> 
            </div></td>
            <td width="33%"></td>
            ...
            <td width="37%"><div align="center">
              <input name="button2" type="button" class="boton_email" id="button2" value="Exportar tiquete de servicio en firme a excel" onclick="exporta_tiquete_excel_administrador()" />
            </div></td>
