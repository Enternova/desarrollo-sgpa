...
<tr>
  ...
  <table width="99%" border="0" cellpadding="2" cellspacing="2" class="tabla_lista_resultados">
                    <tr>
                        <td colspan="8" align="center"  class="fondo_3">Lista de las Solicitudes NO Agendados a Ningun Comit&eacute;</td>
                    </tr>
                    <tr >
                        <td width="10%" align="center" class="fondo_3">N&uacute;mero de la Solicitud</td>
                        <td width="11%" align="center" class="fondo_3">Tipo de Permiso</td>
                    ...
                    $sel_item = traer_fila_row(query_db("select id_item,objeto_solicitud, estado, t1_tipo_proceso_id, ob_solicitud_adjudica, contrato_id from $pi2 where id_item=".$sel_sin_comi[0]));
...
$muestra_fila=1;
					...
					if($tipo_permiso == 1 and $permi_ad == 2){
                    $muestra_fila=2;
                    }
					if($tipo_permiso == 2 and $permi_ad == 1){
                    $muestra_fila=2;
                    }
...
$trm_item=trm_presupuestal($sel_sin_comi[4]);
                            ...
                            ?></td>
                        <td align="center"><? echo saca_nombre_lista($g13,$sel_item[3],'nombre','t1_tipo_proceso_id'); ?></td>
	...
	if($alerta_contrato_vencido != ""){
		echo $alerta_contrato_vencido;
						...
						?>
                            ...
                            $comite_comentario = traer_fila_row(query_db("select * from $c5 where id_comite = $id_comite and id_item = $sel_item[0]"));
                            if($comite_comentario){
                            ?>
                            <strong align="center"class="windowPopup" onClick='window.parent.document.getElementById("div_carga_busca_sol").style.display = "block";
