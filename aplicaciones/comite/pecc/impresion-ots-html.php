...
$id_item_pecc = elimina_comillas(arreglo_recibe_variables($_GET["id_item_pecc"]));
	...
	$sel_datos_contrato = traer_fila_row(query_db("select consecutivo,creacion_sistema, apellido, contratista, id from t7_contratos_contrato where id = ".$sel_contrato_id[0]));
	...
	$numero_contra = numero_item_pecc_contrato($numero_contrato1,$numero_contrato2,$numero_contrato3,$numero_contrato4, $sel_datos_contrato[4]);
...
$sele_presupuesto = query_db("select $pi8.t2_presupuesto_id,$pi8.ano, $g15.nombre, $pi8.adjunto,$pi8.valor_usd,$pi8.valor_cop from $pi8, $g15 where $pi8.t2_item_pecc_id ='".$id_item_pecc."' and $pi8.permiso_o_adjudica = 1 and $g15.t1_campo_id = $pi8.t1_campo_id");
  ...
  $clase="";
        	...
        	while($sel_presu = traer_fila_db($sele_presupuesto)){
				...
				$trm_aplica = trm_presupuestal($sel_item[17]);
			...
			$total_equivale_usd_1 = ($valor_total_cop / trm_presupuestal($sel_item[17])) +$valor_total_usd ;
...
$id_rol_profesional = 8;

$sel_item_obs = traer_fila_db(query_db("select CAST(destino_ots AS text), CAST(duracion_ots AS text), CAST(objeto_solicitud AS text), fecha_inicio_ot, fecha_fin_ot from $pi2 where id_item=".$id_item_pecc));	
/* - Verifica si es una OT y es de servicios con contrato de bienes con el fin de seleccionar el profesional de CYC 2 para enviar en la firma*/
if($sel_item[6] == 8 and $sel_item[4] == 1){
	...
	if($selec_tipo_contras[0] > 0){
		$id_rol_profesional = 39;
		}
}
/* - Verifica si es una OT y es de servicios con contrato de bienes con el fin de seleccionar el profesional de CYC 2 para enviar en la firma*/
		
	$sele_profecional_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol = ".$id_rol_profesional." and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1 and t2_agl_secuencia_solicitud.id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud order by t2_agl_secuencia_solicitud.id_secuencia_solicitud desc"));
	
	if($id_rol_profesional == 39 and ($sele_profecional_fecha[0]=="" or $sele_profecional_fecha[0]==0) ){// si no encuentra profesional 39 vuelva a buscar en los profesionales 8
		$id_rol_profesional = 8;
		$sele_profecional_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol = ".$id_rol_profesional." and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1 and t2_agl_secuencia_solicitud.id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud order by t2_agl_secuencia_solicitud.id_secuencia_solicitud desc"));
		
		}	
	$sele_solicitante_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol = 15 and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1 and t2_agl_secuencia_solicitud. id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud"));

	$rol_gerente_ot_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol = 34 and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1 and t2_agl_secuencia_solicitud. id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud"));
	
	$sele_jefe_area_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol = 9 and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1 and  t2_agl_secuencia_solicitud. id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud"));
	
	if($sele_jefe_area_fecha[0]==""){// si es SUPERINTENDENTE
	$sele_jefe_area_fecha = traer_fila_row(query_db("select t2_agl_secuencia_solicitud_aprobacion.fecha, t2_agl_secuencia_solicitud_aprobacion.id_us from t2_agl_secuencia_solicitud, t2_agl_secuencia_solicitud_aprobacion where  t2_agl_secuencia_solicitud.id_item_pecc = ".$id_item_pecc." and t2_agl_secuencia_solicitud.id_rol in (35,45) and t2_agl_secuencia_solicitud.estado = 1 and t2_agl_secuencia_solicitud_aprobacion.aprobado=1  and t2_agl_secuencia_solicitud. id_secuencia_solicitud = t2_agl_secuencia_solicitud_aprobacion.id_secuencia_solicitud"));
	$nombre_super_o_jefe = "Jefatura de";
	}else{
...
/*
	...
	*/		
			...
			$id_gerente = $rol_gerente_ot_fecha[1];
			$fecha_gerente = "<br />Fecha de Firma: <br />".$rol_gerente_ot_fecha[0]."<br />Firma Virtual en el SGPA";
...
<tr>
  ...
  <table style=" border: solid 1px #ccc; width: 100%" >
    <tr>
      ...
      <td style=" width: 29%; text-align:left; border: solid 0px #ccc;"><span class="datos1">
	  <? echo saca_nombre_lista("t1_area",$sel_item[5],'nombre_html','t1_area_id');?></span></td>
    ...
    <tr>
     ...
     <? if($sel_item_obs[1]=="" or $sel_item_obs[1]==" " or $sel_item_obs[1]==NULL or $sel_item_obs[1]=="NULL"){ ?>
    <tr>
     <td align="right"  ><strong class="sub_titulo">Fecha Inicio:</strong></td>
      <td colspan="3" align="left" ><span class="datos1"><?=$sel_item_obs[3]?></span></td>
      </tr>
      <tr>
     <td align="right"  ><strong class="sub_titulo">Fecha Fin:</strong></td>
      <td colspan="3" align="left" ><span class="datos1"><?=$sel_item_obs[4];?></span></td>
      </tr>
      <?
      	}else{ ?>
      <tr>
  ...
  <table style="width: 100%" >
    <tr>
      <td style="text-indent: 10mm; border: solid 1px #ccc; width: 100%"><span class="datos1"> <strong>Asignado a:</strong> <? echo saca_nombre_lista_ot_pdf("t1_proveedor",$sel_datos_contrato[3],'razon_social','t1_proveedor_id'); ?></span> </td>
    </tr>
  </table>
  <table style="width: 100%" >
    <tr>
  ...
  ?>
    ...
    <tr>
      ...
      <td colspan="2" class="datos"><?=number_format($total_equivale_usd_1)?>
...
if($sel_item_num_nivel[0]<>""){
	$ampl.= "Ampliaci&oacute;n Relacionada: ".numero_item_pecc($sel_item_num_nivel[1],$sel_item_num_nivel[2],$sel_item_num_nivel[3]).", Nivel de Aprobaci&oacute;n ".nivel_aprobacion_solicitud($sel_item_num_nivel[0],"adjudicacion")."<br />";
	$valida_nivel =nivel_aprobacion_solicitud($sel_item_num_nivel[0],"adjudicacion");
}
...
if($ampl==""){
	...
	$sel_item_num_nivel = traer_fila_row(query_db("select id_item, num1,num2,num3 from t2_item_pecc where id_item = ".$sel_item[26]));
	
	$ampl.= "Solicitud que genero el contrato: ".numero_item_pecc($sel_item_num_nivel[1],$sel_item_num_nivel[2],$sel_item_num_nivel[3]).", Nivel de Aprobaci&oacute;n ".nivel_aprobacion_solicitud($sel_item_num_nivel[0],"adjudicacion")."<br />";
	$valida_nivel = nivel_aprobacion_solicitud($sel_item_num_nivel[0],"adjudicacion");
	}
  ...
  <table height="226" style="width: 100%; border: solid 1px #CCC; alignment-adjust:central">
    ...
    <tr class="titulo_lista">
      ...
      <td  style="width:25%;" align="center">Gerente
        de la <br />
        Orden de Trabajo</td>
      <td style="width:25%;" align="center"><?=$nombre_super_o_jefe?><br /><? echo saca_nombre_lista("t1_area",$sel_item[5],'nombre_html','t1_area_id');?></td>
      <td style="width:25%;" align="center">Contratista / <br>
        Representante Legal</td>
      </tr>
    <tr>
      <td class="datos1"><br />Fecha de Firma: <br /><?=$sele_profecional_fecha[0]?><br />Firma Virtual en el SGPA</td>
      <td class="datos1"><?=$fecha_gerente?></td>
      <td class="datos1"><br />Fecha de Firma: <br /><?=$sele_jefe_area_fecha[0]?><br />Firma Virtual en el SGPA</td>
      <td>&nbsp;</td>
      </tr>
    <tr class="datos">
      <td height="34" class="datos1"><strong class="sub_titulo">Nombre:</strong><br>      &nbsp;<? echo saca_nombre_lista($g1,$sele_profecional_fecha[1],'nombre_administrador','us_id');?></td>
      <td class="datos1"><strong class="sub_titulo">Nombre:<br>
		...
		$id_gerente_validacion = $id_gerente;
