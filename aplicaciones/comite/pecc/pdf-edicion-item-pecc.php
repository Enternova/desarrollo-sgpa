...
$trm_actual=trm_presupuestal($sel_item[17]);
	...
	$complemento_reclasificaicon=" and (al_valor_inicial_para_marco is null or al_valor_inicial_para_marco = 0)";
							if($sel_item[6] == 12){
							$complemento_reclasificaicon = " and al_valor_inicial_para_marco is null";	
							$permiso_o_adjudica=1;
							}
	echo "select sum(valor_usd), sum(valor_cop), (sum(valor_cop) / $trm)+sum(valor_usd) from $vpeec22 where id_item = ".$id_item_pecc." and permiso_o_adjudica = ".$permiso_o_adjudica." $complemento_reclasificaicon ) group by trm";
	
	$valor_solicitud = traer_fila_row(query_db("select sum(valor_usd), sum(valor_cop), (sum(valor_cop) / $trm)+sum(valor_usd) from $vpeec22 where id_item = ".$id_item_pecc." and permiso_o_adjudica = ".$permiso_o_adjudica." $complemento_reclasificaicon  group by trm"));
	?>
...
<table width="90%" border="0" align="center" class="tabla_principal">
  <tr>
    <td colspan="3"><img src="https://abastecimiento.hocol.com.co/sgpa/imagenes/coorporativo/logo-cliente-blanco.jpg" alt="" width="130" height="68"></td>
  </tr>
  <tr>
    <td colspan="3" align="center" class="titulo1"><strong>
      <?=saca_nombre_lista($g12,$sel_item[5],'nombre','t1_area_id');?>
      -
      <?=numero_item_pecc($sel_item[16],$sel_item[17],$sel_item[18])?>
    </strong></td>
  </tr>
  <tr>
  ...
  <?
	...
	?>
  <tr>
    <td colspan="3"  class="titulo2"><strong>
      <? if($sel_item[49]==3)  echo "Detalle del valor solicitado para reclasificar"; else echo "Detalle del Monto Solicitado"; ?>
    </strong></td>
  </tr>
  <tr>
    <td colspan="3"><table width="98%" border="0" align="center"  class="tabla_sub6">
      <tr>
  ...
  <tr>
    <td colspan="3" ><table width="98%" border="0" align="center"  class="tabla_proveedores">
      ...
      <?
			...
			?>
      <tr class="<?=$clase?>">
        <td align="center"><?=$se_prove[1]?>
          <strong><font color="#33CC33">
