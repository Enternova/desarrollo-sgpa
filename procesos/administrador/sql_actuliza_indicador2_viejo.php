...
//$desde_cuando = " id_item >= 8838";//primer id item del 2017
//$desde_cuando = " (id_item IN (11802))";//item especificos

/*----------------------------------------------- tiempos congelados ----------------------------------------*/

$sel_item_ini = query_db("select id_item, num3, estado from t2_item_pecc where estado <> 33 and de_historico is null and (tiempos_estandar is null or tiempos_estandar =2) and $desde_cuando order by id_item ");
...
$sel_item = query_db("select id_item, num3, estado, fecha_fin_reprogramada, fecha_ultima_gestion from t2_item_pecc where estado <> 33 and de_historico is null and (tiempos_estandar is null or tiempos_estandar =2) and ".$desde_cuando." order by id_item ");
...
$sel_actividades_resumen = query_db("select actividad_estado,tiempo,fecha_se_requiere,fecha_real,tiempo_para_actividad,actividad_estado_id,dias_reales, estado from $vpeec3 where id_item =".$id_item_pecc." and actividad_estado_id not in (12.1, 19) and aplica_item = 1 order by actividad_estado_id");

$sql_query = "select sum(tiempo), sum(dias_reales), sum(dias_congelado) from $vpeec3 where id_item =".$id_item_pecc." and actividad_estado_id NOT IN (12.1, 19) and   aplica_item = 1";
	...
	$primera_actividad = 0;
	  ...
	  $sel_ultima_gestion="";
	  if($sel_it[2]<>32){
		  if($fecha_hoy >$fecha_reprograma){ $estado_at = "Atrasado";}
		  if($fecha_hoy <=$fecha_reprograma){ $estado_at = "A tiempo";}		  
		  }else{
			  $sel_ultima_gestion = traer_fila_row(query_db("select max(fecha_real) from t2_nivel_servicio_gestiones where id_item = ".$id_item_pecc." and estado = 1"));
			    if($sel_ultima_gestion[0] >$fecha_reprograma){ $estado_at = "Atrasado";}
		  		if($sel_ultima_gestion[0] <=$fecha_reprograma){ $estado_at = "A tiempo";}
			  
			  }
	   
		  
	    
	  if($fecha_reprograma=="" and $sel_ultima_gestion[0]==""){
	  $estado_at="";
	  }
...
$sel_item = query_db("select id_item, num3, estado from t2_item_pecc where estado <> 33 and de_historico is null and (tiempos_estandar is null or tiempos_estandar =2) and ".$desde_cuando." order by id_item ");
/*Este es para actualizar todos los procesos desde el 2103*/
...
$dias_reales=0;
	...
	$dias_congelado=0;
	  ...
	  if($sel_it[2]<=$ult_act_aba){
		  if($fecha_hoy >$fecha_reprograma){ $estado_at = "Atrasado";}
		  if($fecha_hoy <=$fecha_reprograma){ $estado_at = "A tiempo";}		  
		  }else{
			  $sel_ultima_gestion = traer_fila_row(query_db("select max(fecha_real) from t2_nivel_servicio_gestiones where id_item = ".$id_item_pecc." and estado = 1 and t2_nivel_servicio_actividad_id <=".$ult_act_aba));
			    if($sel_ultima_gestion[0] >$fecha_reprograma){ $estado_at = "Atrasado";}
		  		if($sel_ultima_gestion[0] <=$fecha_reprograma){ $estado_at = "A tiempo";}
