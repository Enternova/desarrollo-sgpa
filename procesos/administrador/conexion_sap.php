...
while ($archivo = readdir($directorio)){
	...
	if($tipo_permiso_adj <> 0){
		$sle_id_sql = query_db("select id, tipo from t2_archivos_sap where numero = '".$numero."'");
		
		while($sle_id = traer_fila_db($sle_id_sql)){//limpieza de registros anteriores por numero de documento		
		
		if($sle_id[1] == "Solped"){
			$s_aprob_solped = traer_fila_row(query_db("select count(*) from t2_archivos_sap_solped where usuario like '%E%' and id_archivo_sap = '".$sle_id[0]."'"));	
			if($s_aprob_solped[0]==0){
			...
			}elseif($sle_id[1] == "Pedido"){
		$s_aprob_pedido = traer_fila_row(query_db("select count(*) from t2_archivos_sap_pedido where usuario like '%E%' and id_archivo_sap = '".$sle_id[0]."'"));
		if($s_aprob_pedido[0]==0){
		...
		$sle_id = traer_fila_row(query_db("select id, tipo from t2_archivos_sap where numero = '".$numero."'"));
...
if($tipo_permiso_adj ==1){
		$num_colum_item_mat_prov = 23;
		$columna_despues_de_texto_original = 13;
	}
if($tipo_permiso_adj ==2){
		$num_colum_item_mat_prov = 20;
		$columna_despues_de_texto_original = 17;
	}
/*Cuenta cuantas columnas*/
$numero_columnas = count($data);
$tiene_caracteres_especiales = "NO";
if($numero_columnas > 22 and $tipo_permiso_adj ==2){// ventidos son el numero de columnas, a pezar que en el archivo se ven solo 20
	$tiene_caracteres_especiales = "SI";
	$num_colum_item_mat_prov = $numero_columnas - 2;
	$columna_despues_de_texto_si_mal = $numero_columnas - 5;
	}
/*Cuenta cuantas columnas*/

echo " Correcta ".$columna_despues_de_texto_original." Mal:".$columna_despues_de_texto_si_mal."<br />";
foreach($data as $row) {

echo "Campo $i: $row fila $fila<br>\n"; // Muestra todos los campos de la fila actual 

if($tipo_permiso_adj ==1 and $i==$num_colum_item_mat_prov){
		$num_item = $row;
	}
if($tipo_permiso_adj ==2 and $i==$num_colum_item_mat_prov){
		$num_item = $row;
	}
...
if($tiene_caracteres_especiales == "SI"){
	if($i >= $columna_despues_de_texto_original and $i < $columna_despues_de_texto_si_mal){
		//bloqueo para que no ingrese nada de esas columnas
	}else{	
...
//$sel_usuario_aprueba = traer_fila_row(query_db("select * from t1_usuarios where codigo_sap like '%".$cod_usaurio_aprueba."%'"));
...
//					$upda_item = query_db("update $pi2 set estado=32 where id_item=".$sel_ap[0]);
	...
	$hora_log = date("G:i:s");
	  ...
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, '$Observacion', '$observa ', '', 2)");
							...
							if($acabo_firmas=="SI"){//si ya se aprobo por completo ADJUDICACOIN
			
			$sel_si_es_modificacion = traer_fila_row(query_db("select es_modificacion, id_solicitud_relacionada, estado, num1, num2, num3 from t2_item_pecc where id_item = ".$id_item_pecc));
				if($sel_si_es_modificacion[0]==1){//si es de modificacion finalice la original
					$upda_item = query_db("update $pi2 set estado=32 where id_item=".$sel_si_es_modificacion[1]);
					
					
						$insert = "insert into tseg8_log (id_tipo_log, id_tipo_log_sub_ventana, id_proceso, estado_actual_proceso, estado_resultado, id_us, fecha, hora_seg) values (42, 0,$sel_si_es_modificacion[1], 0, 32, '18463', '".$fecha."', '".$hora_log."')";
	
		$sql_ex=query_db($insert.$trae_id_insrte);
	  $id_log = id_insert($sql_ex);
	  
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, 'Finalización Automatica de la Solicitud y sus Urnas debido a la modificación', '$fecha', 't1_us_usuarios', 1)");
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, 'Modificación Aprobada', '".numero_item_pecc($sel_si_es_modificacion[3], $sel_si_es_modificacion[4], $sel_si_es_modificacion[5])." ', '', 2)");
					
$host_mys="localhost";
$usr_mys="us_hocol-bogota";
$pwd_mys="942ASvle*P";
$dbbase_mys="db_enterproc_hocol";			
	
	$link = mysql_connect($host_mys,$usr_mys, $pwd_mys);
				mysql_select_db($dbbase_mys, $link);

			/*Finaliza las urnas EN LA URNA VIRTUAL*/
				$inserta_proveedor = mysql_query("update pro1_proceso set tp1_id=8 where cd_id_entrega_documentos='".$sel_si_es_modificacion[1]."'");
					}
...
//$insert = "insert into $pi16 (id_secuencia_solicitud, id_us,fecha, aprobado,observacion) values (".$sel_secuencia[0].",".$sel_usuario_que_firma[2].", '$fecha', ".$accion_aprueba.",'".$observa."')";
	...
	if($id_ingreso_firma_adj >0 and $id_ingreso_firma_adj != "" ){//si creo la firma siga el proceso
	  ...
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, '$Observacion', '$observa ', '', 2)");
		...
		if($acabo_firmas=="SI"){//si ya se aprobo por completo PERMSO
	  ...
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, 'Finalizacion Automatica de la Solicitud y sus Urnas debido a la modificacion', '$fecha', 't1_us_usuarios', 1)");
	  $insert = query_db("insert into tseg9_log_detalle (id_log, campo_imprime, detalle, tabla_id, orden) values ($id_log, 'Modificacion Aprobada', '".numero_item_pecc($sel_si_es_modificacion[3], $sel_si_es_modificacion[4], $sel_si_es_modificacion[5])." ', '', 2)");
...
/*Limpia FTP*/
$conexion_id = ftp_connect("192.168.100.38"); // creamos un ID de conexión al servidor

$resultado = ftp_login($conexion_id,"SAP","FTPs4ph0c0l$"); // iniciamos sesion con usuario y contraseña

if((!$conexion_id) || (!$resultado)){

die("No se pudo conectar al FTP para su limpieza, Falló la conexión");
...
if ( $archivo_enviado ) {echo "archivo subido";}else {echo "No se pudo enviar el archivo ";}
<br />
*/
}
